"""Shared effective-score read model (module-external).

The effective score for an answer is
``COALESCE(GradingResult.final_score, StudentAnswer.score)`` with ``max_score``
taken from the ``Question`` table and the raw answer key normalised to the
canonical student via the shared identity resolver. This is the single public
entry point for effective-score computation and is consumed by every analytics
service (summary / distribution / ranking / trend / report / export) plus the
analytics AI tools.

It lives in the module-external services layer so the cross-module model imports
...
cross-module coupling (D-03F). ``edu_cloud.modules.analytics`` re-exports
``get_effective_scores`` / ``get_effective_scores_batch`` for backward
compatibility, so existing import sites keep working unchanged.
"""
