...
class JointExamService:
    ...
    async def add_participant(self, exam_id: str, school_id: str) -> JointExamParticipant:
        await self._get_exam(exam_id)  # verify exam exists
    ...
    async def upload_template(
        ...
        # Validate subject_code is in exam subjects
    ...
    async def submit_scores(
        ...
        # Upsert: select→update/insert (compatible with SQLite)
        count = 0
        ...
        # Check auto-complete: all participants × all subjects submitted?
    ...
    async def _check_auto_complete(self, exam: JointExam) -> None:
        """Check if all participants have uploaded all subjects → auto-complete."""
        participants = (await self.db.execute(
            select(JointExamParticipant)
