"""Student identity resolution for analytics.

Scan data can store either the canonical ``students.id`` or an external
barcode/student key in ``student_answers.student_id``. Analytics must group by
the canonical student whenever it can be resolved, while still preserving the
raw key for audit and unmatched-row warnings.
"""
