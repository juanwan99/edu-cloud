...
必须返回如下格式的 JSON（不要 markdown 围栏，不要用 box_2d）：
...
四种类型：
- choice_group：选择题涂卡区（含规则排列的气泡/圆圈）
  补充字段: rows(题数), cols(选项数,通常4-5), start_no(起始题号), qg_indexno(组序号从1开始), score(每题分数,无则0)
- essay：主观题/解答题作答区域（大面积空白供书写）
  单题: qno(整数)+score
  多题共用一个框: qnos(整数数组)+scores(数组)+splits(分割比例数组,0到1,长度=题数+1)
  例: "qnos":[3,4,5],"scores":[4,4,6],"splits":[0,0.25,0.55,1.0] 表示Q3占25%高度,Q4占30%,Q5占45%
  注意: splits 要根据每题实际答题空间大小估算，不要平均分
- barcode：学生信息区域（含条形码/二维码、姓名、准考证号、座位号等），每张卡通常有且仅有 1 个
- not_a_region：非作答区（卡头标题/定位黑块/注意事项/缺考标记等，不含条形码）

重要规则：
- 同一大题可能被栏线分割为多个区域（如 Q17 左栏和中栏各一部分），这些区域都标 essay 并使用相同 qno
- 如果区域顶部有说明文字（如"请在各题目的答题区域内作答"）但下方有空白答题区，仍应标为 essay，不要标 not_a_region
- 每个 R?? 必须出现在输出中
- 选择题起始题号通常为 1"""
...
def _resolve_image(p: str) -> Path:
    ...
    if p.startswith("/home/ops/projects/edu-cloud/"):
        return Path(p)
...
async def auto_detect_cv_regions(req: AutoDetectCVRequest) -> dict:
    ...
    labels, raw_content = await _llm_label(img_path, rects, w, h, bubble_hints)
