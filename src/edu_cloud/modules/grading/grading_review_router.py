...
@router.get("/question/{question_id}/student-answers")
async def get_question_student_answers(
    question_id: str,
    db: AsyncSession = Depends(get_db),
    current: dict = Depends(get_current_user),
):
    """获取某题下所有学生答案及其 AI 评分结果（供前端阅卷结果展示用）。"""
    ...
    results_map: dict[str, GradingResult] = {}
    if answer_ids:
        results = (await db.execute(
            select(GradingResult).where(GradingResult.answer_id.in_(answer_ids))
        )).scalars().all()
        results_map = {r.answer_id: r for r in results}

    items = []
    for a in answers:
        gr = results_map.get(a.id)
        grading_result = None
        if gr:
            details = None
            if gr.ai_raw_response and isinstance(gr.ai_raw_response, dict):
                details = gr.ai_raw_response.get("details")
            grading_result = {
                "id": gr.id,
                ...
                "ai_confidence": gr.ai_confidence,
                "status": gr.status,
                "source": gr.source,
                "details": details,
            }
        image_url = None
        if a.image_path:
            if a.image_path.startswith("/uploads/"):
                image_url = a.image_path
            elif a.image_path.startswith(str(Path(settings.UPLOAD_DIR).resolve())):
                image_url = "/uploads/" + a.image_path.split(str(Path(settings.UPLOAD_DIR).resolve()) + "/", 1)[-1]
            else:
                image_url = f"/api/v1/scan/pipeline/scan-image?path={a.image_path}"
        items.append({
            "id": a.id,
    ...
    logger.debug("get_question_student_answers: question=%s, count=%d", question_id, len(items))
    return items
