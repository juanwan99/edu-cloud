...
RUBRIC_GENERATION = build_rubric_generation(
    ROLE,
    "生物注意概念层级：完整等值简称放 equivalentAnswers；上位词、日常说法放 scoringRules 给部分分；结构/物质/过程混淆放 exclusionRules。不同生物学过程（如蒸发vs蒸腾）不得放入 equivalentAnswers，应在 scoringRules 中按语境给分"
)
...
5. **生物学科等价/近似表述**：
   ...
   - **不同生物学过程不可直接等价**：如"蒸发"（物理过程）与"蒸腾"（生物过程）是不同概念，除非 rubric 明确声明在本题语境下可互换
...
""" + GRADING_METHOD_BASE + """
   ...
   - 若rubric已说明"序号对应某结构"，学生写结构名称且无歧义可按等价处理；OCR重复字（如"受精精"）不扣分
   - **概念边界严判**：不同生物学概念不作满分等价（如"蒸发"和"蒸腾"是不同过程、"结构"和"物质"不可互换），除非 rubric 明确声明允许
