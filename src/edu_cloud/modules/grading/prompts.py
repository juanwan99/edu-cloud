...
3. 直接返回纯 JSON（禁止 markdown 代码块）：{"score": 数字, "feedback": "评语", "confidence": 0-1的置信度}
4. score 不能超过满分，不能为负数
5. confidence 表示你对评分的确信程度
6. feedback 控制在 50 字以内"""
...
5. 直接返回纯 JSON（禁止 markdown 代码块）：{"score": 数字, "feedback": "评语", "confidence": 0-1}
6. score 不超过满分、不为负
7. feedback 控制在 30 字以内"""
...
6. 直接返回纯 JSON（禁止 markdown 代码块）：{"score": 数字, "feedback": "评语", "confidence": 0-1}
7. score 不超过满分、不为负
8. feedback 控制在 80 字以内"""
