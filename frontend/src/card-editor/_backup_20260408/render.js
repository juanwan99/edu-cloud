...
function renderSingleBlank(b, regionId, si, bi) {
  ...
  return `<div class="essay-line" data-region="${regionId}" data-sub="${si}" data-blank="${bi}" style="width:${b.w}">${answer}</div>`;
}
...
function renderSubsHTML(subs, regionId, cuts) {
  ...
  for (let si = 0; si < subs.length; si++) {
    ...
    // 将 blanks 分组成行：30% 的短空两两配对同行，48%/100% 独占一行
    // 第一行特殊：带小问标签
    function buildRows() {
      const rows = [];
      let i = 0;
    ...
    for (let ri = 0; ri < rows.length; ri++) {
      ...
      if (ri === 0) {
        ...
        blanksInner += `<div class="essay-first-line${idxs.length > 1 ? ' multi-blank' : ''}">` +
          `<span class="essay-sub-label" contenteditable="true" data-region="${regionId}" data-sub="${si}" data-edit="sub">${subLabel}</span>` +
          cellsHTML + `</div>`;
      } else if (idxs.length > 1) {
        // 多个短空同行
        blanksInner += `<div class="essay-blank-row">` +
          idxs.map(bi => renderBlankWithSep(blanks[bi], bi, bi === blanks.length - 1)).join('') +
          `</div>`;
      } else {
        // 单空独占一行
        blanksInner += renderBlankWithSep(blanks[idxs[0]], idxs[0], idxs[0] === blanks.length - 1);
      }
    }
    const blanksHTML = `<div class="essay-blanks" data-region="${regionId}" data-sub="${si}">${blanksInner}</div>`;

    // flex 值按空行数量加权：空行多的小问获得更多空间
    const subFlex = Math.max(1, blanks.length);
    html += `<div class="essay-sub-block" style="flex:${subFlex} 1 0;" data-region="${regionId}" data-sub="${si}">` +
