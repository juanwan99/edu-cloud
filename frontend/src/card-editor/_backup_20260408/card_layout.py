...
def _estimate_question_height(q_result: dict) -> float:
    ...
    for sub in q_result["subs"]:
        h += SUB_LABEL_H
        blank_count = len(sub.get("blanks", []))
        h += max(1, blank_count) * LINE_H
        h += SUB_GAP
    return h
...
def calculate_layout(parsed_questions: list[dict], config: dict | None = None) -> dict:
    ...
    # ── Step 4: 视觉理想高度计算 ──
    # 每题的理想高度 = 题头 + 各小问(标签 + 横线×理想行高 + 间距) + 分值加成
    for r in results:
        blank_count = sum(len(s.get("blanks", [])) for s in r["subs"])
        sub_count = len(r["subs"])
        ideal = (HEADER_H + ESSAY_PADDING
                 + blank_count * VISUAL_LINE_H
    ...
    for slot in slots:
        ...
        if ideal_total <= col_cap:
            ...
            for it in items:
                ...
                # 保底：题头 + 每空最小 7.5mm
                blank_count = sum(len(s.get("blanks", [])) for s in it["subs"])
                minimum = HEADER_H + blank_count * 7.5 + len(it["subs"]) * (SUB_LABEL_H + SUB_GAP)
