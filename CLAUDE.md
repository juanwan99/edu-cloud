...
## 交付合同（所有验证/完成声明必须对齐此合同）

| 字段 | 值 |
|------|-----|
| 权威代码 | `frontend/src/` → `vite build` → `frontend/dist/` |
| 用户消费 URL | `https://mcu.asia`（nginx 443 → `frontend/dist/`） |
| 验证目标 | `https://mcu.asia`（唯一可作为完成证据的 URL） |
| dev server | `localhost:8080` / `localhost:5173`（仅限调试，不可验收） |
| 后端验证 | `pytest` 测试套件通过（`.venv/bin/python -m pytest`） |

**规则：**
- Playwright 截图如果来自 `localhost:*`，只能用于调试，不能作为完成证据
- 前端变更声称完成前，必须 `vite build` + 确认 `mcu.asia` 可访问
- `npx vite build` 和 `npm run build` 均可，vite plugin 自动修复 dist/ 权限（nginx 可读）
- 后端变更声称完成前，必须有 `pytest` 通过的输出
...
uv sync --extra dev  # 首次或 venv 重建后必须加 --extra dev（安装 pytest-asyncio 等测试依赖）
