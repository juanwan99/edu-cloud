...
> Phase 0.7B（drift burn-down，进行中）：后端中间件门控硬化。①（item3/R5-DC2）`module_middleware.py` 抽 `resolve_module_code`/`_longest_prefix_match`，匹配规则由 dict 插入序首匹配改为**最长前缀优先**，与守卫 `check_module_semantics._actual_gating` 严格同算法（exempt-first → ROUTE_MODULE_MAP 最长前缀；exempt 与 gated 前缀集互斥，重排 inert）——收口规则漂移（守卫绿但运行时重叠前缀可命中错误模块）。②（item4）`ROUTE_MODULE_MAP` 补 `/api/v1/conduct→conduct`、`/api/v1/exam-imports→exam`，收口后端 fail-open drift（参照 0.6C profile；前端已标 moduleCode、authGuard 已 fail-close，后端补同源门控=模块关即不可用）。**academic 不补门控**：前端 `/academic/*` 仅 permission 无 moduleCode（teaching-frontend-unwired），单独后端 gating 会让有 manage_scheduling 但 teaching 关闭的校 403 破坏页面，需前端 wiring 配套（超出 0.7B「不改业务 UI」），保留为 known_drift。③（item5）`EXEMPT_PREFIXES` 补 menus/portal/grades/teachers/client-logs（本就 pass-through，显式豁免，行为零变更）。守卫 stale-drift 检测强制：收口入口须同步删 `backend_routes` drift 字段 + `known_drift` 登记（11→3：academic + studio + teaching）。
