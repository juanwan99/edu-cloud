...
- [x] M-5 约束加固 — `c53572a`（exam/grading_task/grading_result 三表 CHECK 约束）
- [x] M-7 分页 total_count — `c942030`（audit-logs 端点加 total/limit/offset/items 包装）
- [ ] M-8 camelCase 统一（breaking API 变更，需前后端协调）
- [~] M-9 ReviewPage 拆分 — 已有 3 个 composable 拆分，降级为可选
- [x] M-10 data/ 清理 — `c53572a`（删除 3.2MB stale test artifacts）
