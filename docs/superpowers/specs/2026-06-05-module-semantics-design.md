...
> #7 拆"模块码漂移 + 野值"2 测试、#13 拆 a/b 2 测试，故 17 编号 = 19 反例测试；加 4 个正例（self/backend/frontend/portal 各 `*_passes_on_real`）= 端到端 23 测试（详见 plan Task 各 Step 的 passed 计数）。
...
- 19 条反例测试（17 编号）全部真变红，尤其 #2（新 fail-open fail-closed）+ #3（映射错配）+ #17（dashboard route 漂移）。
