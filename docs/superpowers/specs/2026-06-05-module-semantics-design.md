...
3. **前端逐 route 比对**（抓"合法值映射错" + fail-closed，plan-review R2 F1；dashboard route 级比对，本轮必修③）— `check_frontend`
   - 解析 routeAccess + router-meta + sidebar + dashboard 的 (route, moduleCode) 对。
   - **routeAccess / sidebar / dashboard（route 形态干净，作可见性真源）**：每个 route 必须 ∈ `frontend_route_module`，否则 → 红（fail-closed，防"未声明前端入口用合法值"）；route 已声明且真源非 null 时值不一致 → 红（含 `/analytics→exam` 错配、sidebar/dashboard 错配到另一合法值）。**dashboard action 带 `route` 字段（实测 `DashboardPage.vue:435,444,455,465,474` 各 action 均 `route:` + `moduleCode:`），故与 sidebar 同等做 route 级比对，不再只做野值检查**——能抓"合法 moduleCode 但 route 映射错"（必修③）。
...
4. **前端 drift 探测**（plan-review F2，实现为独立 check）— `check_frontend_drift`：frontend drift（studio/teaching）**不按 id 硬编码放行**——每个须有探测器 `_FRONTEND_DRIFT_PROBES[id](parsed)→bool` 验证实际状态：drift 仍成立→绿；实际已不成立（疑似已修复）→红（应删登记）；无探测器的 frontend drift→红（fail-closed，由第6类收敛兜底）。
...
> #7 拆"模块码漂移 + 野值"2 测试、#13 拆 a/b 2 测试，故 18 编号 = 20 反例测试；加 4 个正例（self/backend/frontend/portal 各 `*_passes_on_real`）= 端到端 24 测试（详见 plan Task 各 Step 的 passed 计数）。
...
- 20 条反例测试（18 编号）全部真变红，尤其 #2（新 fail-open fail-closed）+ #3（映射错配）+ #17（dashboard route 漂移）+ #18（已修复但 drift 保留）。
