...
| AI | AgentProvider 路由层（Coze-first，current_pydantic fallback）+ CozeProvider + Internal Tool Gateway + 67 @edu_tool（16 模块）+ PolicyToolGuardrail 4 层 RBAC + DataScope/capability/module 过滤 + AgentBudget + ConfirmationBroker 写确认 + ArtifactManager DB 持久化 + TraceRecorder DB 双写。Coze chat 已接入；Coze 工具选择/调用尚未产品化，当前 `required_action` submit/resume 未验证可用，下一步应走 HTTP plugin callback 独立落地；旧 Pydantic EduAgentRuntime 保留为短期 fallback | 真实 Coze bot/token/gateway 运行态配置 |
