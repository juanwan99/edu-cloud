...
| `scripts/governance/check_module_semantics.py` + `docs/governance/module-semantics.yaml` + `frontend/src/router/index.js` | candidate-active | Phase 0.5 static module-semantics guard + Phase 0.6 runtime authGuard module gating. Both R2 findings fixed (`f51342a` drift fail-closed denominator, `8606ac6` authGuard direct-URL gating). Pending: re-run `codex-review f82df2a..HEAD` to PASS before close-out — see NOW.md "Module Governance Phase 0.5 + 0.6". No Portal/Phase 1 until PASS. |
