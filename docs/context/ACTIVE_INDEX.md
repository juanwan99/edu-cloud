...
| `scripts/governance/check_module_semantics.py` + `docs/governance/module-semantics.yaml` + `frontend/src/router/index.js` | candidate-active | Phase 0.5 static guard + Phase 0.6 runtime authGuard gating, main body = 4 commits (`f51342a`/`8606ac6`/`bf421e8`/`bd8be46`), HEAD `bd8be46`. Review reached **R4 FINDINGS**; R4 carved out to coverage-completeness sub-task. See NOW.md "Module Governance Phase 0.5 + 0.6". No Portal/Phase 1 until sub-task review PASSES. |
| `docs/plans/2026-06-06-phase06-coverage-handoff.md` | candidate-active | Phase 0.6 coverage-completeness sub-task handoff. Root-cause fix for R4 F-002 (guard must enforce moduleCode on every controlled route) + priority security item F-001 (`/profile/student/:studentId` direct-URL fail-open). Goal/Must Preserve/Must Not Change inside. |
