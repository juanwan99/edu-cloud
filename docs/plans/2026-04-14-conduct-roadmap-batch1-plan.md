...
- conduct 后端: `cd /home/ops/projects/edu-cloud-t2 && .venv/bin/python -m pytest tests/test_conduct/ -q` → 68 passed, exit 0
- services: `cd /home/ops/projects/edu-cloud-t2 && .venv/bin/python -m pytest tests/test_services/test_school_settings_service.py tests/test_services/test_homework_permissions.py -q` → 15 passed, exit 0
- frontend conduct 3 件套: `cd /home/ops/projects/edu-cloud-t2/frontend && npx vitest run src/__tests__/sidebarConfig.conduct.test.js src/__tests__/AppSidebar.test.js src/pages/parent/__tests__/ParentRules.spec.js` → 13 passed

**退出条件:**
- 5 Tasks 全部 completed，conduct 后端 ≥ 78 passed (68 基线 + ~10 Task 增量：T4 +3 / T1 +4 / T2 +3)，frontend conduct ≥ 22 passed
...
|---|---|---|---|
| T5 文档数字修正 | — | `/home/ops/projects/edu-cloud-t2/CLAUDE.md` + `docs/plans/2026-04-13-conduct-next-phase-handoff.md` | — |
...
> **T-Wipe 2026-04-18 已覆盖**: T-Wipe Phase 3 (commit 6d4126e) 已将 CLAUDE.md
> 的 conduct 历史漂移数字对齐 ECS 实测 baseline = 68 passed @ 2026-04-18。
> 本 Task 降级为"post-T-Wipe 文档一致性验证 + 必要时补漏"。

**Files:**
- Verify-only: `/home/ops/projects/edu-cloud-t2/CLAUDE.md`（T-Wipe 已清洗）
- Verify-or-clean: `docs/plans/2026-04-13-conduct-next-phase-handoff.md`（handoff 文件，L018 Phase 6 统一处理）
...
Run:
```bash
cd /home/ops/projects/edu-cloud-t2 && .venv/bin/python -m pytest tests/test_conduct/ -q --tb=no 2>&1 | tail -3
```
...
Run:
```bash
grep -nE "conduct.*\b(1[0-2][0-9])\b" /home/ops/projects/edu-cloud-t2/CLAUDE.md
```
Expected: 无输出（T-Wipe Phase 3 已清零 conduct 区三位数基线数字）。若有残留，升级为补丁修复。

- [ ] **Step 1.3: 检查 conduct-next-phase-handoff 残留**

Run:
```bash
grep -nE "\b(1[0-2][0-9])\s+passed|\b1[0-2][0-9]\s*\+\s*1[0-2]" docs/plans/2026-04-13-conduct-next-phase-handoff.md 2>/dev/null
```
- 有残留 → 判断文件是否 pre-takeover（git log --diff-filter=A 早于 `00cfc3d` 2026-04-16）：
...
- [ ] **Step 1.5: Commit（若 Step 1.4 触发）**

```bash
cd /home/ops/projects/edu-cloud-t2 && git reset HEAD >/dev/null 2>&1 && \
  git add docs/plans/2026-04-13-conduct-next-phase-handoff.md && \
  git diff --cached --name-only
# 验证 staged 只有涉及文件

git commit -m "$(cat <<'EOF'
docs(conduct): T5 post-T-Wipe 文档一致性验证

T-Wipe Phase 3 (6d4126e) 已清洗 CLAUDE.md conduct 区数字。
T5 补齐 conduct-next-phase-handoff 残留（archived marker 或 ECS 数字）。
...
若 Step 1.3 无残留，跳过 commit，直接进入 Task 2（T4）。
...
Run:
```bash
cd /home/ops/projects/edu-cloud-t2 && grep -n "__tablename__" src/edu_cloud/modules/conduct/models.py
```
...
Run:
```bash
cd /home/ops/projects/edu-cloud-t2 && grep -n "name=\"" src/edu_cloud/ai/tools/conduct.py | grep -v description
```
...
Run:
```bash
cd /home/ops/projects/edu-cloud-t2 && python -m pytest tests/test_conduct/test_module_governance.py -v 2>&1 | tail -15
```
...
```bash
cd /home/ops/projects/edu-cloud-t2 && python scripts/governance/aggregate_modules.py 2>&1 | tail -20
```
...
Run:
```bash
cd /home/ops/projects/edu-cloud-t2 && python -m pytest tests/test_conduct/ -q --tb=no 2>&1 | tail -5
```
Expected: `71 passed`（68 基线 + 3 governance 新增）。

- [ ] **Step 2.9: Commit**

```bash
cd /home/ops/projects/edu-cloud-t2 && git reset HEAD >/dev/null 2>&1 && \
  git add src/edu_cloud/modules/conduct/MODULE.md tests/test_conduct/test_module_governance.py && \
  git diff --cached --name-only
# staged 应该只有这 2 个文件
```
...
- Test: `tests/test_conduct/test_permissions.py`（新增 ≥3 测试）
- Test: `frontend/src/__tests__/permissions.lesson_prep.test.js`（新建）
...
- [ ] **Step 3.1: 写失败后端测试（test_permissions.py 新增 3 测试）**
...
Run:
```bash
cd /home/ops/projects/edu-cloud-t2 && python -m pytest tests/test_conduct/test_permissions.py::test_lesson_prep_leader_no_view_conduct tests/test_conduct/test_permissions.py::test_lesson_prep_leader_no_manage_conduct -v 2>&1 | tail -10
```
...
Run:
```bash
cd /home/ops/projects/edu-cloud-t2 && python -m pytest tests/test_conduct/test_permissions.py -v 2>&1 | tail -15
```
...
```bash
cd /home/ops/projects/edu-cloud-t2 && git reset HEAD >/dev/null 2>&1 && \
  git add src/edu_cloud/core/permissions.py tests/test_conduct/test_permissions.py && \
  git diff --cached --name-only
```
```bash
git commit -m "$(cat <<'EOF'
...
- 新增 4 个红测：lesson_prep 无 VIEW/MANAGE，subject_teacher/homeroom 未误伤
...
Run:
```bash
cd /home/ops/projects/edu-cloud-t2/frontend && npx vitest run src/__tests__/permissions.lesson_prep.test.js 2>&1 | tail -20
```
...
Run:
```bash
cd /home/ops/projects/edu-cloud-t2/frontend && npx vitest run src/__tests__/permissions.lesson_prep.test.js 2>&1 | tail -10
```
Expected: 5 passed。
...
```bash
cd /home/ops/projects/edu-cloud-t2 && git reset HEAD >/dev/null 2>&1 && \
  git add frontend/src/config/permissions.js frontend/src/__tests__/permissions.lesson_prep.test.js && \
  git diff --cached --name-only
```
```bash
git commit -m "$(cat <<'EOF'
...
```bash
cd /home/ops/projects/edu-cloud-t2 && python -m pytest tests/test_conduct/ -q --tb=no 2>&1 | tail -3
```
Expected: `75 passed`（68 基线 + 3 governance + 4 T1）。

```bash
cd /home/ops/projects/edu-cloud-t2/frontend && npx vitest run src/__tests__/sidebarConfig.conduct.test.js src/__tests__/AppSidebar.test.js src/pages/parent/__tests__/ParentRules.spec.js src/__tests__/permissions.lesson_prep.test.js 2>&1 | tail -5
```
...
Run:
```bash
cd /home/ops/projects/edu-cloud-t2 && grep -rn "\.date\b" src/edu_cloud/modules/conduct/ | grep -v "\.date_type\|__date__\|ConductRecord.date\|Record\.date\|record\.date\|created_at"
```
记录所有 `AddPointsRequest(...).date` 或 `request.date` 的出现位置。预计在 admin_service.py 1-3 处 / admin_router.py 0-1 处。

Run:
```bash
cd /home/ops/projects/edu-cloud-t2 && grep -rn "\"date\"\\|'date'" frontend/src/api/conduct.js frontend/src/pages/conduct/ConductPoints.vue 2>/dev/null
```
...
Run:
```bash
cd /home/ops/projects/edu-cloud-t2 && python -m pytest tests/test_conduct/test_admin_crud_api.py -k "record_date" -v 2>&1 | tail -15
```
...
```bash
cd /home/ops/projects/edu-cloud-t2 && grep -n "data\.date\|request\.date" src/edu_cloud/modules/conduct/admin_router.py src/edu_cloud/modules/conduct/admin_service.py
```
...
Run:
```bash
cd /home/ops/projects/edu-cloud-t2 && python -m pytest tests/test_conduct/test_admin_api.py -k "record_date or add_points" -v 2>&1 | tail -20
```
Expected: 3 新测试 PASS；其他既有 add_points 测试（用 today default）维持 PASS。

Run（全量 conduct 回归）:
```bash
cd /home/ops/projects/edu-cloud-t2 && python -m pytest tests/test_conduct/ -q --tb=short 2>&1 | tail -10
```
Expected: `78 passed`（75 基线 + 3 T2）。

- [ ] **Step 4.7: 前端同步**

Run 审计：
```bash
cd /home/ops/projects/edu-cloud-t2 && grep -rn "date" frontend/src/api/conduct.js frontend/src/pages/conduct/ConductPoints.vue | grep -v "new Date\|Date\.\|\.toLocaleDateString\|createdAt\|created_at\|成绩日期\|//"
```
...
Run:
```bash
cd /home/ops/projects/edu-cloud-t2/frontend && npx vitest run --run 2>&1 | tail -10
```
...
```bash
cd /home/ops/projects/edu-cloud-t2 && git reset HEAD >/dev/null 2>&1 && \
  git add src/edu_cloud/modules/conduct/schemas.py src/edu_cloud/modules/conduct/admin_service.py tests/test_conduct/test_admin_api.py && \
...
- PointsRecordResponse.date 保留（response 字段，不影响）
- admin_service.py request.date → request.record_date
- frontend api/Vue 同步（若有）
- 新增 3 红测：传 record_date 字符串 / 不传 / 显式 null
...
**Files:**
- Modify: `frontend/src/config/sidebarConfig.js`（CONDUCT_* 三档 → 单一 CONDUCT_ITEMS + filter）
- Test: `frontend/src/__tests__/sidebarConfig.conduct.test.js`（矩阵扩展 9 角色 × 关键入口）
...
⚠ **关键说明**：此 plan 预期 subject_teacher 从 2 项扩展到 4 项（view+manage 权限派生）。这是 design.md §3.4 中 R-T3-followup 标注的"待 Plan Review 二次确认"项。如果 Plan Review 阶段用户要求保守（subject_teacher 维持 2 项），需在 CONDUCT_ITEMS 声明模型中加 `allowed_roles` 白名单字段，测试断言随之调整。

- [ ] **Step 5.2: 跑测试确认 FAIL（至少 academic_director / grade_leader / subject_teacher 三个测试）**

Run:
```bash
cd /home/ops/projects/edu-cloud-t2/frontend && npx vitest run src/__tests__/sidebarConfig.conduct.test.js 2>&1 | tail -25
```
...
Run:
```bash
cd /home/ops/projects/edu-cloud-t2/frontend && npx vitest run src/__tests__/sidebarConfig.conduct.test.js 2>&1 | tail -15
```
Expected: 原 13 + 新 9 = 22 passed。

- [ ] **Step 5.5: 跑 AppSidebar.test.js + 全量前端**

```bash
cd /home/ops/projects/edu-cloud-t2/frontend && npx vitest run src/__tests__/AppSidebar.test.js 2>&1 | tail -10
```
Expected: 既有测试 PASS（AppSidebar.vue 接收 sidebarConfig 产物，不因 CONDUCT_ITEMS_* 重构而 break）。

```bash
cd /home/ops/projects/edu-cloud-t2/frontend && npx vitest run 2>&1 | tail -5
```
...
若 dev server 运行中：
```bash
cd /home/ops/projects/edu-cloud-t2/frontend && npm run dev
# 访问 http://0.0.0.0:8080，用 academic_director 账号登录（如 admin + switch-role）
...
```bash
cd /home/ops/projects/edu-cloud-t2 && git reset HEAD >/dev/null 2>&1 && \
  git add frontend/src/config/sidebarConfig.js frontend/src/__tests__/sidebarConfig.conduct.test.js && \
  git diff --cached --name-only
```
...
新增 9 个矩阵测试覆盖 9 角色 × 关键入口。22 passed (13+9)。

L017 behavior_change R-T3：用户 2026-04-14 明确批准（"两个都批准"）
隐含 R-T3-followup（subject_teacher 2→4 扩展）留 Plan Review 独立确认。
...
- ✓ subject_teacher 渲染 4 个 conduct 菜单项（T3 扩展，待二次确认）
...
**Files:**
- Create: `docs/plans/2026-04-14-conduct-roadmap-batch1-state.json`
- Modify: `docs/plans/2026-04-14-conduct-roadmap-design.md`（§ 10 填 [实现完成]）
- Modify: `/home/ops/projects/edu-cloud-t2/CLAUDE.md`（进行中 → 已完成设计）
...
cd /home/ops/projects/edu-cloud-t2 && python -m pytest tests/test_conduct/ -q --tb=no 2>&1 | tail -3
```
Expected: `78 passed`（68 + 3 T4 + 4 T1 + 3 T2 = 78）。

Run:
```bash
cd /home/ops/projects/edu-cloud-t2 && python -m pytest tests/test_services/test_school_settings_service.py tests/test_services/test_homework_permissions.py -q 2>&1 | tail -3
```
Expected: `15 passed`。
...
cd /home/ops/projects/edu-cloud-t2/frontend && npx vitest run src/__tests__/sidebarConfig.conduct.test.js src/__tests__/AppSidebar.test.js src/pages/parent/__tests__/ParentRules.spec.js src/__tests__/permissions.lesson_prep.test.js 2>&1 | tail -5
```
Expected: `27 passed`（13 基线 + 5 T1 frontend + 9 T3 frontend = 27）。

- [ ] **Step 6.2: 生成 state.json**

使用 Write 工具创建 `docs/plans/2026-04-14-conduct-roadmap-batch1-state.json`：
...
```json
{
  ...
  "tasks": [
    ...
    {"id": "6", "desc": "批次 1 收尾", "status": "in_progress"}
  ],
  "test_baseline": {
    "conduct_backend": 78,
    "services": 15,
    "frontend_conduct_suite": 27
  },
  "updated_at": "<填 Step 6.5 时跑的 date '+%Y-%m-%d %H:%M:%S' 结果>"
}
```

（updated_at 先占位，Step 6.5 的 commit 前再 Edit 填真实时间戳）

- [ ] **Step 6.3: 在 design.md §10 填 [实现完成]**

Run:
```bash
cd /home/ops/projects/edu-cloud-t2 && git log --oneline --grep="T[1-5]" -10
```
...
> [2026-MM-DD HH:MM:SS 批次 1 实现完成] Commits: <first>..<last> | Tests: conduct=78, services=15, frontend_conduct_suite=27
>
...
使用 Edit 工具，`/home/ops/projects/edu-cloud-t2/CLAUDE.md`:
...
```markdown
- **德育板块统筹规划路线图 批次 1 [实现完成]**: R3 PASS 后的全景治理批次 1 — T5 文档 post-T-Wipe 验证 / T4 conduct MODULE.md 补全 / T1 lesson_prep_leader 权限回收（C 方案，behavior_change R-T1）/ T2 AddPointsRequest.date→record_date（修 pydantic v2 field-type shadowing bug，behavior_change R-T2）/ T3 sidebar 按 permissions 派生（academic_director 入口 3→7，grade_leader 3→5，subject_teacher 2→4 per R-T3-followup，behavior_change R-T3）。5 Tasks / 7 commits / Gate 1+2 PASS。测试分项：conduct 78 / services 15 / frontend conduct suite 27（ECS pytest 实测 baseline 68 + T1/T2/T4 共 10 增量）。批次 2 (D-005/D-009/D-010 运维就绪) 与批次 3 (D-006/D-007/D-008 真实验证) 占位等待启动。真源: `edu-cloud/docs/plans/2026-04-14-conduct-roadmap-design.md`
```
...
测试总计: conduct 78 / services 15 / frontend 27（分项列示，不汇总）。
...
- R-T3-followup（subject_teacher 2→4）在 Gate 1 Plan Review 阶段独立确认

---

## R1 Findings 处置总结（2026-04-14 Gate 1 Plan Review）

Gate 1 R1 结论: FAIL（9 finding，HIGH×4 / MED×5，raw log SHA256 `9554e5ee...`）。处置如下：

| ID | Sev | Cat | Type | 状态 | 处置位置 |
|---|---|---|---|---|---|
| F001 | HIGH | code-bug | defect_fix | resolved-inline | Task 4 Files + Step 4.5 已补 `admin_router.py:115,137`，service 层已是目标状态无需改 |
| F002 | HIGH | test-gap | defect_fix | resolved-inline | Task 4 Step 4.2 测试代码改为入口级 POST 200 + DB readback，fixture 改 `homeroom_headers`，测试文件改 `test_admin_crud_api.py` |
| F003 | MED | code-bug | defect_fix | resolved-inline | Task 2 Step 2.7 脚本路径 `scripts/governance/aggregate_modules.py` |
| F004 | MED | code-bug | defect_fix | resolved-inline | 见下方 § state.json 生命周期 |
| F005 | HIGH | design-concern | **behavior_change** | **approved** | 用户 2026-04-14 07:45:00 精确回复"批准 F005"，subject_teacher 2→4 扩展批准落地，plan 现状维持；gates.json F005.approval 已记录 |
| F006 | MED | test-gap | defect_fix | resolved-inline | 见下方 § 入口级测试补充 |
| F007 | MED | test-gap | defect_fix | resolved-inline | 见下方 § CONDUCT_ITEMS 治理测试 |
| F008 | HIGH | design-concern | defect_fix | resolved-inline | 见下方 § Contract Pack |
| F009 | MED | code-bug | defect_fix | resolved-in-design | `docs/plans/2026-04-14-conduct-roadmap-design.md` §8 已修，承认 workspace CLAUDE.md 为跨 repo audit trail 合法产出 |
...
每个 Task 的 Steps 增加 2 个 state.json 管理 step（入口 + 出口）。对现有 Step 编号影响：
- Task 1 (T5): 原 Step 1.1~1.6 保持 + 新增 Step 1.0（in_progress）+ Step 1.7（completed）
- Task 2 (T4): + Step 2.0 + Step 2.10
- Task 3 (T1): + Step 3.0 + Step 3.12
- Task 4 (T2): + Step 4.0 + Step 4.10
- Task 5 (T3): + Step 5.0 + Step 5.8（加 F007 + F006 测试后）
- Task 6 保持，但 6.2 改为"更新"而非"创建" state.json
...
### § Contract Pack（F008 修复，按 `~/.claude/config/contract-pack-schema.md`）

```yaml
contract_pack:
  invariants:
    - id: INV-T1-001
      rule: "lesson_prep_leader 永不拥有 VIEW_CONDUCT 或 MANAGE_CONDUCT"
    ...
    - id: INV-T1-002
      rule: "subject_teacher 与 homeroom_teacher 未被 T1 误伤（仍拥有 conduct 基础权限）"
      verification:
        type: new_test
    ...
    - id: INV-T1-003
      rule: "lesson_prep_leader 调 conduct API 入口被拒（403），ToolAccessResolver 过滤 6 conduct AI tools"
    ...
    - id: INV-T2-001
      rule: "AddPointsRequest 的公开字段名不与 datetime.date 类型名冲突"
    ...
    - id: INV-T2-002
      rule: "AddPointsRequest 的 record_date 传入值真实落库到 ConductRecord.date（入口级契约）"
      verification:
        type: new_test
        test_ref: "tests/test_conduct/test_admin_crud_api.py::test_add_points_with_record_date_field (DB readback)"
    - id: INV-T3-001
      rule: "sidebar conduct 菜单项的可见性由 permission 派生，不由角色-档硬编码"
      rationale: "三档 FULL/VIEWER/TEACHER 粒度过粗，教务主任/年级组长有权无路"
      verification:
        type: new_test
        test_ref: "frontend/src/__tests__/sidebarConfig.conduct.test.js (9 角色 × 菜单项矩阵)"
    - id: INV-T3-002
      rule: "CONDUCT_ITEMS[*].perm 字段必须来自 ROLE_PERMISSIONS 定义的 permission 集"
    ...
    - id: INV-T3-003
      rule: "academic_director 登录后侧边栏 conduct 段渲染出 7 项（入口级）"
      verification:
        type: new_test
        test_ref: "frontend/src/__tests__/AppSidebar.test.js (academic_director 7 项入口级)"
    - id: INV-T4-001
      rule: "conduct MODULE.md 的 owns_tables 与 models.py __tablename__ 集合严格一致"
      verification:
        type: new_test
        test_ref: "tests/test_conduct/test_module_governance.py::test_owns_tables_matches_orm_definitions"
    - id: INV-T4-002
      rule: "conduct MODULE.md 的 depends_on.ai_tools 与 ai.registry 实际注册的 conduct tools 严格一致"
      verification:
        type: new_test
  ...
  counter_examples:
    - id: CE-001
      ...
      tests_that_still_pass: "INV-T1-001 的 has_permission 断言仍 PASS（frozenset/list 都支持成员判定，但差集 `-` 会对 list FAIL）"
      mitigation: "批次 2 D-005 加 sentinel: `assert isinstance(_TEACHER_BASE, (set, frozenset))` 或改为显式过滤"
      status: "deferred → 批次 2 D-005"
    - id: CE-002
      scenario: "开发者在 CONDUCT_ITEMS 新增菜单项时 perm 字段拼错（如 manage_condut）"
      break_invariant: INV-T3-002
      tests_that_still_pass: "如果没有 INV-T3-002 治理测试，9 角色 × 矩阵快照测试会显示该项对所有角色不可见——误判为'设计如此'"
    ...
    - id: CE-003
      ...
      tests_that_still_pass: "如果只跑 service 层单测或直接调用 service.add_points(record_date=...)，发现不了 router 层 data.record_date 属性缺失的 AttributeError"
  ...
  test_debt:
    - id: TD-001
      ...
      reason: "归属批次 3 D-006；批次 1 scope 外"
      deadline: "2026-05-15（批次 3 启动时）"
    - id: TD-002
    ...
    - id: TD-004
      ...
      deadline: "2026-05-01（批次 2 启动时）"
    - id: TD-005
      item: "T2 record_date 业务校验（不应晚于今日或早于班级学期起始）"
      reason: "业务校验留未来增强，当前允许任意合法日期"
      deadline: "deferred（无阻塞需求）"
...
Plan saved to `/home/ops/projects/edu-cloud-t2/docs/plans/2026-04-14-conduct-roadmap-batch1-plan.md`.
