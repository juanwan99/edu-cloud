...
**退出条件:**
- 5 Tasks 全部 completed，conduct 后端 ≥ 130 passed（R1-F006 入口级 + R5-F001 对照组补齐后），frontend conduct ≥ 29 passed
...
Run:
```bash
cd C:/Users/Administrator/edu-cloud && python -m pytest tests/test_conduct/test_admin_crud_api.py::test_lesson_prep_leader_cannot_call_conduct_api tests/test_conduct/test_admin_crud_api.py::test_subject_teacher_with_same_scope_passes_rbac -v 2>&1 | tail -10
```
...
cd C:/Users/Administrator/edu-cloud && python -m pytest tests/test_conduct/ -q --tb=no 2>&1 | tail -3
```
Expected: `127 passed`（118 基线 + 3 T4 governance + 4 T1 helper + 1 T1 入口级 API 403 + 1 T1 对照组 subject_teacher 200 = 127；Task 4/5 尚未跑，T2 入口级 3 测试不在此 Task 3 回归里）。
...
```
Expected: `130 passed`（118 基线 + 3 T4 + 4 T1 helper + 1 T1 入口级 API 403 + 1 T1 对照组 subject_teacher 200（R5-F001）+ 3 T2 入口级 = 130）。
...
Run:
```bash
cd C:/Users/Administrator/edu-cloud/frontend && npx vitest run src/__tests__/AppSidebar.conduct.test.js 2>&1 | tail -10
```
...
Run:
```bash
cd C:/Users/Administrator/edu-cloud/frontend && npx vitest run src/__tests__/sidebarConfig.conduct.test.js src/__tests__/AppSidebar.test.js src/__tests__/AppSidebar.conduct.test.js 2>&1 | tail -15
```
...
cd C:/Users/Administrator/edu-cloud && python -m pytest tests/test_conduct/ -q --tb=no 2>&1 | tail -3
```
Expected: `130 passed`（118 基线 + 3 T4 governance + 4 T1 helper + 1 T1 入口级 API 403 + 1 T1 对照组 subject_teacher 200（R5-F001）+ 3 T2 入口级 = 130）。
...
```
Expected: `29 passed`（13 基线 3 件套 + 10 T3 sidebarConfig 新增（9 矩阵 + 1 F007 perm 合法性）+ 1 T3 AppSidebar.conduct.test.js F006 入口级 + 5 T1 permissions.lesson_prep = 29；AppSidebar.test.js 原 3 保持）。conduct 后端套件配套 `130 passed`。
...
2. `test_baseline.conduct_backend`：`118` → `130`
...
> [2026-04-14 HH:MM:SS 批次 1 实现完成] Commits: <first>..<last> | Tests: conduct=130, services=15, frontend_conduct_suite=29
>
...
测试总计: conduct 130 + services 15 + frontend 29 = 174 passed。
