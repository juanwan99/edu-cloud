...
**退出条件:**
- 5 Tasks 全部 completed，conduct 后端 ≥ 125 passed，frontend conduct ≥ 22 passed
...
cd C:/Users/Administrator/edu-cloud && git reset HEAD >/dev/null 2>&1 && \
  git add CLAUDE.md docs/plans/2026-04-13-conduct-next-phase-handoff.md && \
...
Run:
```bash
cd C:/Users/Administrator/edu-cloud && python -m pytest tests/test_conduct/test_admin_api.py -k "record_date or add_points" -v 2>&1 | tail -20
```
...
```
Expected: `128 passed`（125 基线 + 3 T2）。
...
Run:
```bash
cd C:/Users/Administrator/edu-cloud/frontend && npx vitest run src/__tests__/sidebarConfig.conduct.test.js 2>&1 | tail -15
```
...
cd C:/Users/Administrator/edu-cloud && python -m pytest tests/test_conduct/ -q --tb=no 2>&1 | tail -3
```
Expected: `128 passed`（118 + 3 T4 + 4 T1 + 3 T2 = 128）。
...
> [2026-04-14 HH:MM:SS 批次 1 实现完成] Commits: <first>..<last> | Tests: conduct=128, services=15, frontend_conduct_suite=27
>
...
测试总计: conduct 128 + services 15 + frontend 27 = 170 passed。
