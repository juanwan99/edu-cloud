...
### 3.1 修改的 Nginx 配置
`/etc/nginx/conf.d/[已清理-旧配置]`（已备份到 `[已清理-旧配置].bak-2026-04-16`）
...
**测试完成后请恢复**：
```bash
sudo cp /etc/nginx/conf.d/[已清理-旧配置].bak-2026-04-16 /etc/nginx/conf.d/[已清理-旧配置]
sudo nginx -s reload
```

已有的 `edu.[已清理-旧域名]` server block（L33）是生产形态（静态 dist + /api 反代），后续接入正式域名时启用。
...
`/etc/nginx/conf.d/subdomains.conf` + `[已清理-旧配置]` 已配置：
- `paper.[已清理-旧域名]` → 127.0.0.1:8200（paper-skill）
- `zhixue.[已清理-旧域名]` → 127.0.0.1:3000
- `class-points.[已清理-旧域名]` → 127.0.0.1:8500
- `zhcps.[已清理-旧域名]` → ?
- `bio.[已清理-旧域名]`、`kb.[已清理-旧域名]`
- `edu.[已清理-旧域名]` → 静态 dist + /api→9000（生产形态）
- `api.[已清理-旧域名]` → ?

DNS：`[已清理-旧域名]` 解析到 Cloudflare（104.21.85.32 / 172.67.201.191）；子域名需用户在 Cloudflare 后台加 A 记录指向 `47.121.197.52`。
