...
updated: 2026-04-18 (T-Wipe ECS-rewrite)
status: approved
T-level: T3
...
**基线（ECS pytest 实测 @ 2026-04-18 10:25:04）:**
- conduct 后端: `cd /home/ops/projects/edu-cloud && .venv/bin/python -m pytest tests/test_conduct/ -q` → 68 passed, exit 0
- services: `cd /home/ops/projects/edu-cloud && .venv/bin/python -m pytest tests/test_services/test_school_settings_service.py tests/test_services/test_homework_permissions.py -q` → 15 passed, exit 0
- frontend conduct 3 件套: `cd /home/ops/projects/edu-cloud/frontend && npx vitest run src/__tests__/sidebarConfig.conduct.test.js src/__tests__/AppSidebar.test.js src/pages/parent/__tests__/ParentRules.spec.js` → 13 passed

**退出条件:**
- 5 Tasks 全部 completed，conduct 后端 ≥ 80 passed（68 ECS 基线 + ~12 增量：T4 +3 / T1 +4 helper / T1 +1 API 403 / T1 +1 对照组 / T2 +3 入口级；R1-F006 / R5-F001 保留语义），frontend conduct ≥ 29 passed
...
|---|---|---|---|
| T5 文档 post-T-Wipe 验证 | — | `/home/ops/projects/edu-cloud/CLAUDE.md`（T-Wipe 已清洗，仅验证）+ `docs/plans/archived/2026-04/2026-04-13-conduct-next-phase-handoff.md`（Phase 6 或本 Task 按 L018 处置） | — |
...
## Task 1: T5 文档 post-T-Wipe 一致性验证（热身，0-1 commits）

> **T-Wipe 2026-04-18 已覆盖**: T-Wipe Phase 3 (master commit 6d4126e) 已把 CLAUDE.md
> 所有 conduct 历史漂移数字对齐 ECS 实测 baseline = 68 passed @ 2026-04-18。
> W4 worktree 的 CLAUDE.md 尚未应用 T-Wipe Phase 3（本会话 W4 Phase 4.3 处理），
> Executor 执行本 Task 前应确认 W4 CLAUDE.md 已对齐 ECS 基线。

**Files:**
- Verify: `/home/ops/projects/edu-cloud/CLAUDE.md`（post-T-Wipe 数字已对齐，本 Task 仅校验）
- Verify-or-clean: `docs/plans/archived/2026-04/2026-04-13-conduct-next-phase-handoff.md`（handoff 文件；Phase 6 处置或本 Task 补漏）

**性质:** 纯文档验证，非 behavior_change。T1 级别，无测试契约要求。

**依赖:** 无（T-Wipe 已做前置清洗）。Contract 保留 R1-F004 state.json 生命周期。
...
```bash
cd /home/ops/projects/edu-cloud && python3 -c "
...
- [ ] **Step 1.1: 预验证 ECS pytest baseline**

Run:
```bash
cd /home/ops/projects/edu-cloud && .venv/bin/python -m pytest tests/test_conduct/ -q --tb=no 2>&1 | tail -3
```
Expected: 最后一行 `68 passed`（退出码 0）。若数字漂移，暂停本 plan 排查。

- [ ] **Step 1.2: 验证 CLAUDE.md post-T-Wipe 一致性**

Run:
```bash
grep -nE "conduct.*\b1[0-2][0-9]\b" /home/ops/projects/edu-cloud/CLAUDE.md
```
Expected: 无输出（T-Wipe 已清零 conduct 区三位数基线漂移数字）。若有残留，升级为补丁修复。

- [ ] **Step 1.3: 检查 conduct-next-phase-handoff 残留**

Run:
```bash
grep -nE "\b1[0-2][0-9]\s+passed|\b1[0-2][0-9]\s*\+\s*1[0-2]" docs/plans/archived/2026-04/2026-04-13-conduct-next-phase-handoff.md 2>/dev/null
```
- 有残留 → 判断文件 pre-takeover（git log --diff-filter=A 早于 `00cfc3d` 2026-04-16）：
  - pre-takeover → 顶部 5 行加 `<!-- pre-takeover: archived for history, not active spec -->` marker
  - post-takeover → Edit 改为 ECS 实测数字 68
- 无残留 → 跳过 Step 1.4

- [ ] **Step 1.4: 若有残留，按 Step 1.3 决策执行修复**

参考 L018 铁律：清除而非订正，禁留"旧数字 → 新数字"对比表。
...
- [ ] **Step 1.6: Commit（若 Step 1.4 触发 + state.json 置 completed）**

```bash
cd /home/ops/projects/edu-cloud && git reset HEAD >/dev/null 2>&1 && \
  git add docs/plans/archived/2026-04/2026-04-13-conduct-next-phase-handoff.md docs/plans/2026-04-14-conduct-roadmap-batch1-state.json && \
  git diff --cached --name-only
# 若 Step 1.4 未触发，仅 staged state.json

git commit -m "$(cat <<'EOF'
docs(conduct): T5 post-T-Wipe 文档一致性验证 + state.json Task1 completed

T-Wipe master 6d4126e 已清洗 CLAUDE.md conduct 数字。
W4 Phase 4.3 同步后，T5 补齐 conduct-next-phase-handoff 残留（archived marker 或 ECS 数字）
+ state.json Task 1 pending → completed。
...
Expected: HEAD commit message 含 "T5 post-T-Wipe 验证"。若 Step 1.3 无残留，commit 仍发生（state.json Task 1 status 迁移）。

**审查清单:**
- ✓ CLAUDE.md grep conduct + 三位数基线 无命中
- ✓ conduct-next-phase-handoff 残留已处置（ECS 数字 / archived marker / 无）
- ✓ state.json Task 1 = completed
- 关键行为：任何后续 planner 读 conduct 数字只得 ECS 实测 68
...
- Test: `tests/test_conduct/test_module_governance.py`（新建，3 测试：test_module_md_exists / test_owns_tables_matches_orm_definitions / test_exposes_ai_tools_matches_registry）
...
Run:
```bash
cd /home/ops/projects/edu-cloud && grep -n "__tablename__" src/edu_cloud/modules/conduct/models.py
```
...
Run:
```bash
cd /home/ops/projects/edu-cloud && grep -n "name=\"" src/edu_cloud/ai/tools/conduct.py | grep -v description
```
...
service 层（AdminService / ParentService / RulesService / ExportService）
       │
       ▼
ORM 8 表（student_profiles ... conduct_semesters）
       │
...
Run:
```bash
cd /home/ops/projects/edu-cloud && python -m pytest tests/test_conduct/test_module_governance.py -v 2>&1 | tail -15
```
...
```bash
cd /home/ops/projects/edu-cloud && python scripts/governance/aggregate_modules.py 2>&1 | tail -20
```
...
Run:
```bash
cd /home/ops/projects/edu-cloud && python -m pytest tests/test_conduct/ -q --tb=no 2>&1 | tail -5
```
Expected: `71 passed`（68 ECS 基线 + 3 governance 新增）。
...
- Test: `tests/test_conduct/test_admin_crud_api.py`（**R1-F006 入口级 + R5-F001 对照组** 新增 2 测试：`test_lesson_prep_leader_cannot_call_conduct_api` → API 403（RBAC 回收）+ `test_subject_teacher_with_same_scope_passes_rbac` → 200（同 scope 对照组，root-cause 区分）
...
Run:
```bash
cd /home/ops/projects/edu-cloud && python -m pytest tests/test_conduct/test_permissions.py::test_lesson_prep_leader_no_view_conduct tests/test_conduct/test_permissions.py::test_lesson_prep_leader_no_manage_conduct -v 2>&1 | tail -10
```
...
Run:
```bash
cd /home/ops/projects/edu-cloud && python -m pytest tests/test_conduct/test_permissions.py -v 2>&1 | tail -15
```
Expected: 所有 test PASS（包括新增 4 个 T1 测试 + 原有测试）。

- [ ] **Step 3.4a: 新增 T1 入口级 API 403 + 对照组测试（R5-F001 避免 scope 假绿）**

**R5-F001 根因**：`api/permissions.py:18-22` 的 `get_visible_class_ids(role)` 对非管理员角色返回 `role.class_ids or []`；`conduct/permissions.py:47-51` 的 `check_class_scope` 对空 visible 直接 403。若测试不给 lesson_prep_leader 赋 `class_ids`，即使 RBAC 没改（VIEW_CONDUCT 还在），API 也会因 scope 403 → 测试假绿，INV-T1-003 契约失真。

**修复**：fixture 给 lesson_prep_leader 赋 `class_ids=[cls.id]` 让 scope 通过，从而隔离测试"纯 RBAC 层的 403"。再加对照组：同 class_ids 下 subject_teacher 应 200（证明 scope 链路正常，403 只来自 RBAC）。

使用 Edit 工具在 `tests/test_conduct/test_admin_crud_api.py` 末尾追加。**严格复用 `conftest.py` 的 import 路径和 helper 模式**：

```python


# ── T1 (R1-F006 入口级 + R2-F001 import 修正 + R5-F001 scope 隔离): ──
# lesson_prep_leader 调 conduct API 返回 403（纯 RBAC 层，非 scope 假绿）

@pytest.mark.anyio
async def test_lesson_prep_leader_cannot_call_conduct_api(
    client, db, school_class_student,
):
    """T1 入口级: lesson_prep_leader 即便拥有 class scope 也被 RBAC 拒（403）."""
    import uuid
    ...
    role = UserRole(
        ...
        school_id=school.id,
        is_primary=True,
        class_ids=[cls.id],  # R5-F001: 给 class scope，隔离掉 scope 403，让测试只验 RBAC
    )
    db.add(role)
    await db.commit()
...
async def test_subject_teacher_with_same_scope_passes_rbac(
    ...
    """T1 入口级对照组 (R5-F001): 同 class_ids 下 subject_teacher 有 VIEW_CONDUCT → 200，
    证明上一个 test 的 403 来自 RBAC（VIEW_CONDUCT 被 T1 回收）而非 scope 空导致。"""
    import uuid
    from edu_cloud.models.user import User
    from edu_cloud.models.user_role import UserRole
    from edu_cloud.shared.auth import create_access_token
...
**R2-F001 修复说明**：原草稿 import `from edu_cloud.modules.student.models import UserRole, User` 错——实际 `User` 在 `src/edu_cloud/models/user.py`、`UserRole` 在 `src/edu_cloud/models/user_role.py`。修正为与 `tests/test_conduct/conftest.py:1-7` 一致的导入路径；`set_password()` / `create_access_token({sub, role})` 也按现有 harness 模式落地。

- [ ] **Step 3.4b: 跑入口级测试 + 对照组确认 PASS**

Run:
```bash
cd /home/ops/projects/edu-cloud && python -m pytest tests/test_conduct/test_admin_crud_api.py::test_lesson_prep_leader_cannot_call_conduct_api tests/test_conduct/test_admin_crud_api.py::test_subject_teacher_with_same_scope_passes_rbac -v 2>&1 | tail -10
```
Expected: 2 tests PASS。lesson_prep_leader 被 RBAC 403；subject_teacher 同 scope 下 200。这对 INV-T1-003 构成 root-cause 区分证据。
...
- 新增 2 个入口级红测（R1-F006 + R5-F001 对照组）：
  - test_lesson_prep_leader_cannot_call_conduct_api → API 403（同 scope 下 RBAC 拒）
  - test_subject_teacher_with_same_scope_passes_rbac → 200（同 scope 对照组，证明 403 来自 RBAC 回收而非 scope 空）
...
Run:
```bash
cd /home/ops/projects/edu-cloud/frontend && npx vitest run src/__tests__/permissions.lesson_prep.test.js 2>&1 | tail -20
```
...
Run:
```bash
cd /home/ops/projects/edu-cloud/frontend && npx vitest run src/__tests__/permissions.lesson_prep.test.js 2>&1 | tail -10
```
Expected: 5 passed。
...
```bash
cd /home/ops/projects/edu-cloud && python -m pytest tests/test_conduct/ -q --tb=no 2>&1 | tail -3
```
Expected: `77 passed`（68 ECS 基线 + 3 T4 governance + 4 T1 helper + 1 T1 入口级 API 403 + 1 T1 对照组 subject_teacher 200 = 77；Task 4/5 尚未跑，T2 入口级 3 测试不在此 Task 3 回归里）。

```bash
cd /home/ops/projects/edu-cloud/frontend && npx vitest run src/__tests__/sidebarConfig.conduct.test.js src/__tests__/AppSidebar.test.js src/pages/parent/__tests__/ParentRules.spec.js src/__tests__/permissions.lesson_prep.test.js 2>&1 | tail -5
```
Expected: `18 passed`（原 13 + 新 5 permissions.lesson_prep）。**R4-F001：本 Task 3 结束时 `AppSidebar.conduct.test.js` 尚未创建（Task 5 Step 5.2b 才建），所以此命令不含该文件**。
...
4. **API 入口级拒绝（R1-F006 用户可触达入口）**
   ...
   - 命令: `pytest tests/test_conduct/test_admin_crud_api.py::test_lesson_prep_leader_cannot_call_conduct_api tests/test_conduct/test_admin_crud_api.py::test_subject_teacher_with_same_scope_passes_rbac -v`（含 R5-F001 对照组）

---

## Task 4: T2 AddPointsRequest.date → record_date（后端 + 前端，1 commit）

**Files（R2-F004 范围收窄，后端 API 契约修复）:**
...
- Test: `tests/test_conduct/test_admin_crud_api.py`（新增 3 测试，入口级 POST 200 + DB readback）

**R2-F004 范围收窄（不改前端）**：
- 本批次 T2 仅修复后端 API 契约，**不改** `frontend/src/api/conduct.js` 和 `frontend/src/pages/conduct/ConductPoints.vue`
- 当前 UI 不提供日期补录入口——`ConductPoints.vue:53-76` 无日期控件、API 调用透传也无 `record_date` 字段
- 前端 UX（日期控件 + form.record_date + 提交字段对齐）是独立 behavior_change，需单独 L017 批准，留未来批次（新 TD 项或批次 3 D-006 家长端 E2E 协同）
- 退出语义相应收窄：**POST `/records` API 接受 record_date 并落库 Record.date**（不再承诺"班主任补录昨天积分功能真可用"——该 UX 需 UI 改造）
...
- [ ] **Step 4.1: 审计后端 request.date 使用点（R2-F004 仅后端）**

Run:
```bash
cd /home/ops/projects/edu-cloud && grep -rn "\.date\b" src/edu_cloud/modules/conduct/ | grep -v "\.date_type\|__date__\|ConductRecord.date\|Record\.date\|record\.date\|created_at"
```
记录所有 `AddPointsRequest(...).date` 或 `request.date` 的出现位置。预计 admin_router.py:115,137 两处；admin_service.py 参数名已是 `record_date` 无匹配（verify-only）。

**不审计前端**：R2-F004 收窄后前端不改，跳过 ConductPoints.vue / api/conduct.js 审计。

- [ ] **Step 4.2: 写失败测试（tests/test_conduct/test_admin_crud_api.py 新增 3 测试，R2-F002 文件名锁定）**

使用 Edit 工具在 `tests/test_conduct/test_admin_crud_api.py` 末尾追加（按现有 test 风格；该文件已有 `homeroom_teacher` / `homeroom_headers` fixture）：
...
Run:
```bash
cd /home/ops/projects/edu-cloud && python -m pytest tests/test_conduct/test_admin_crud_api.py -k "record_date" -v 2>&1 | tail -15
```
...
```bash
cd /home/ops/projects/edu-cloud && grep -n "data\.date\|request\.date" src/edu_cloud/modules/conduct/admin_router.py src/edu_cloud/modules/conduct/admin_service.py
```
...
Run:
```bash
cd /home/ops/projects/edu-cloud && python -m pytest tests/test_conduct/test_admin_crud_api.py -k "record_date or add_points" -v 2>&1 | tail -20
```
Expected: 3 新测试 PASS；其他既有 add_points 测试（用 today default）维持 PASS。

Run（全量 conduct 回归）:
```bash
cd /home/ops/projects/edu-cloud && python -m pytest tests/test_conduct/ -q --tb=short 2>&1 | tail -10
```
Expected: `80 passed`（68 ECS 基线 + 3 T4 + 4 T1 helper + 1 T1 入口级 API 403 + 1 T1 对照组 subject_teacher 200（R5-F001）+ 3 T2 入口级 = 80）。

- [ ] **Step 4.7: 前端不改（R2-F004 范围收窄 — 本步骤仅声明不执行）**

**R2-F004 明确决定**：本批次 T2 不改前端；跳过 api/conduct.js 和 ConductPoints.vue 的任何编辑。Executor 若发现前端未传 `record_date`，**不视为 bug**——UI 日期输入入口是独立 behavior_change，留未来批次。仅在 commit message 中说明"后端 API 契约修复；前端 UX 补录日期留未来批次"。

- [ ] **Step 4.8: 前端不改（R2-F004 范围收窄）— 跳过前端测试同步**

本 Task 不触发任何 frontend vitest 变化；T3 的 sidebar 测试由 Task 5 独立覆盖。
...
```bash
cd /home/ops/projects/edu-cloud && git reset HEAD >/dev/null 2>&1 && \
  git add src/edu_cloud/modules/conduct/schemas.py \
          src/edu_cloud/modules/conduct/admin_router.py \
          tests/test_conduct/test_admin_crud_api.py \
          docs/plans/2026-04-14-conduct-roadmap-batch1-state.json && \
  git diff --cached --name-only
# staged 4 个文件（R2-F004 范围收窄后）：schemas.py + admin_router.py + 1 test + state.json
# 禁止 staged：admin_service.py（verify-only）/ frontend/src/api/conduct.js（R2-F004 不改）/ frontend/src/pages/conduct/ConductPoints.vue（R2-F004 不改）
```
...
修复（R2-F004 范围收窄，后端 API 契约修复，不改前端）:
...
- 不改 frontend/src/api/conduct.js 和 frontend/src/pages/conduct/ConductPoints.vue
  （R2-F004：UI 日期补录控件独立 behavior_change，需单独 L017 批准，留未来批次）
...
- ✗ schemas.py 中 PointsRecordResponse.date 字段名被错误改名 → response 字段丢失（前端/测试 KeyError）
- ✗ admin_service 中 Record.date SQLAlchemy column 被错误改名 → ORM AttributeError
- ✗ 前端 ConductPoints.vue 被擅自 edit 接入 record_date → 超出 R2-F004 收窄范围，需 L017 新批准
- **关键行为（收窄）**：POST `/records` API 接受 `record_date` 字段并落库 `ConductRecord.date`；前端 UX 日期控件补录不在本批次范围
...
1. **record_date 字段生效**
   ...
   - 反例: 错误实现仍用旧字段名 `date` → pydantic 忽略 record_date → DB Record.date 存 today → `assert str(rec.date) == "2026-04-10"` FAIL（通过 POST 200 返 created_ids + DB readback 捕捉，非旧式 `assert data[0]["date"]`）
   - 边界: "2026-04-10"（过去）/ null / 未传 / "invalid"（422）
   - 回归: 防止未来再次把字段名改回 `date` 撞 type
   - 命令: `pytest tests/test_conduct/test_admin_crud_api.py::test_add_points_with_record_date_field -v`
...
2. **不传 record_date 默认 today**
   ...
   - 命令: `pytest tests/test_conduct/test_admin_crud_api.py::test_add_points_without_record_date_defaults_today -v`
...
3. **null 等价于不传**
   ...
   - 回归: N/A
   - 命令: `pytest tests/test_conduct/test_admin_crud_api.py::test_add_points_with_record_date_null_defaults_today -v`
...
- Modify: `frontend/src/components/shell/AppSidebar.vue`（**R2-F003 视图层改动**：router-link 增加 `:data-module="item.moduleCode"` 属性以供入口级选择器定位）
- Test: `frontend/src/__tests__/sidebarConfig.conduct.test.js`（矩阵扩展 9 角色 × 关键入口 + R1-F007 `test_conduct_items_perm_valid` 治理契约；Step 5.2a 顶部 import 段新增 `CONDUCT_ITEMS` import，describe 块追加文件末尾）
- Test: `frontend/src/__tests__/AppSidebar.conduct.test.js`（**R2-F003 新建独立文件**：不 mock sidebarConfig，让真实 CONDUCT_ITEMS/filterConductByRole 生效；R1-F006 入口级 `test_academic_director_conduct_seven_items`。原 `AppSidebar.test.js` 顶层 `vi.mock('../config/sidebarConfig.js')` 吃掉真实 config，不能在其末尾追加 conduct 入口级测试）
...
Run:
```bash
cd /home/ops/projects/edu-cloud/frontend && npx vitest run src/__tests__/sidebarConfig.conduct.test.js 2>&1 | tail -25
```
...
- [ ] **Step 5.2a: 新增 CONDUCT_ITEMS.perm 合法性治理测试（R1-F007 入口级，R2-F003 分两段落地）**

**分两段 Edit 防止 ESM 非法（R2-F003 修正）**：原方案写"在末尾追加"但 import 必须在文件顶部；若只在末尾追加导致 `import` 出现在 `describe` 后，Vitest 解析非法。

**Part A**：使用 Edit 工具在 `frontend/src/__tests__/sidebarConfig.conduct.test.js` 顶部 import 段（现有 L10-12 `import { describe, it, expect } from 'vitest'; import { getSidebarItems } from '../config/sidebarConfig.js'; import { ROLE_PERMISSIONS } from '../config/permissions.js'` 之后）追加一行：

```javascript
import { CONDUCT_ITEMS } from '../config/sidebarConfig.js' // R1-F007: 依赖 Step 5.3 export const
```

（注：`ROLE_PERMISSIONS` 已在顶部 import，不重复。）

**Part B**：使用 Edit 工具在同文件**末尾**（现有 `describe('sidebarConfig conduct 挂载契约', ...)` 块之后）追加新 describe 块（不含任何 import 语句）：
...
Run:
```bash
cd /home/ops/projects/edu-cloud/frontend && npx vitest run src/__tests__/sidebarConfig.conduct.test.js -t "CONDUCT_ITEMS perm 合法性" 2>&1 | tail -10
```
Expected: test FAIL（当前 sidebarConfig.js 未 export CONDUCT_ITEMS，import 失败 → 语法解析错 / ReferenceError）。这个测试会在 Step 5.3 完成 export 后 PASS。

- [ ] **Step 5.2b: 新建 AppSidebar.conduct.test.js 独立文件（R1-F006 + R2-F003 修正）**

**R2-F003 根因**：原 `frontend/src/__tests__/AppSidebar.test.js` 顶层已 `vi.mock('../config/sidebarConfig.js', () => ({ getSidebarItems: () => [...4 个 hardcoded items] }))`（见该文件 L7-14），新 conduct 入口级 case 追加到其末尾**不会触达真实** `CONDUCT_ITEMS` / `filterConductByRole`，永远过不了 7 项断言。

**修复**：使用 Write 工具**新建**独立文件 `frontend/src/__tests__/AppSidebar.conduct.test.js`，**不 mock** sidebarConfig，让真实 export 生效。需要 mock 的是 `roles.js` / `vue-router` / `stores/auth.js`（按 AppSidebar 运行期依赖）；vue-router 的 `RouterLink` mock 必须允许 `data-module` 属性透传到渲染 DOM。

参考骨架（Executor 可按 Vitest + @vue/test-utils 惯例微调）：
...
vi.mock('vue-router', () => ({
  useRoute: () => ({ path: '/' }),
  RouterLink: {
    // 允许 data-module 等未声明 attr 透传到渲染 <a>
...
describe('AppSidebar conduct 入口级渲染 (R1-F006 + R2-F003 独立 harness)', () => {
  beforeEach(() => {
    setActivePinia(createPinia())
  })

  it('test_academic_director_conduct_seven_items: academic_director 侧边栏 conduct 段渲染 7 项', async () => {
    ...
    expect(conductItems.length).toBe(7)
  })
})
```

**依赖 Step 5.3 的视图层改动**：`AppSidebar.vue` 的 `<router-link v-for="item in navItems">` 需加 `:data-module="item.moduleCode"` 属性以供选择器定位（R2-F003）。

Run:
```bash
cd /home/ops/projects/edu-cloud/frontend && npx vitest run src/__tests__/AppSidebar.conduct.test.js 2>&1 | tail -10
```
Expected: test FAIL（当前 academic_director 走 VIEWER 档 3 项 + AppSidebar.vue 尚无 data-module 属性，无法 findAll 到 7 项）。Step 5.3 完成 sidebarConfig 派生 + AppSidebar.vue data-module 属性后 PASS。

- [ ] **Step 5.3: 重构 sidebarConfig.js CONDUCT 部分**

使用 Read 工具先完整读 `frontend/src/config/sidebarConfig.js`（约 140 行）。
...
⚠ **R1-F006 + R2-F003 视图层改动（Task 5 Files 已纳入 AppSidebar.vue）**：当前 `frontend/src/components/shell/AppSidebar.vue:10-19` 使用 `<router-link v-for="item in navItems" :key="item.route" :to="item.route">` 渲染菜单项，**不带 data-module 属性**。Step 5.3 同步编辑 AppSidebar.vue 模板，在 `<router-link>` 增加 `:data-module="item.moduleCode"`（Vue Router 4 RouterLink 会把未声明 attr 透传到渲染 `<a>`）。改动后 `wrapper.findAll('[data-module="conduct"]')` 才能定位到 9 个 conduct 项（及 exam/studio/calendar 其他模块项）。

**禁止通过继续扩大 mock、或把 mock 返回值直接改成 7 项来制造假绿**（R2-F003 forbidden pattern）。
...
Run:
```bash
cd /home/ops/projects/edu-cloud/frontend && npx vitest run src/__tests__/sidebarConfig.conduct.test.js src/__tests__/AppSidebar.test.js src/__tests__/AppSidebar.conduct.test.js 2>&1 | tail -15
```
Expected: sidebarConfig.conduct.test.js 原 13 + 新 9 矩阵 + 新 1 (F007 perm 合法性) = 23 passed；AppSidebar.test.js 原 3 保持 PASS（R2-F003 未合并入此文件，独立文件承接 F006 入口级）；AppSidebar.conduct.test.js 新 1 (F006 academic_director 7 项) PASS。合计 27 passed。

- [ ] **Step 5.5: 跑 AppSidebar.test.js + 全量前端**

```bash
cd /home/ops/projects/edu-cloud/frontend && npx vitest run src/__tests__/AppSidebar.test.js 2>&1 | tail -10
```
...
```
Expected: 全量前端测试 PASS（本批次新增 `+16` = 5 T1 permissions.lesson_prep + 10 T3 sidebarConfig（9 矩阵 + 1 F007 治理）+ 1 T3 AppSidebar.conduct。R5-F003 事实校准：repo 全量 frontend vitest 当前 **234 passed**（2026-04-18 实跑，多批次累积），本批次结束后应是 234 + 16 = **250 passed**；"72 原" 是历史过期数字，不再作为口径）。

- [ ] **Step 5.6: 手动浏览器 smoke（可选但推荐）**

若 dev server 运行中：
```bash
cd /home/ops/projects/edu-cloud/frontend && python ~/.claude/scripts/serve.py "C:/Program Files/nodejs/npm.cmd" run dev
...
cd /home/ops/projects/edu-cloud && git reset HEAD >/dev/null 2>&1 && \
  git add frontend/src/config/sidebarConfig.js \
          ...
          frontend/src/__tests__/sidebarConfig.conduct.test.js \
          frontend/src/__tests__/AppSidebar.conduct.test.js \
          docs/plans/2026-04-14-conduct-roadmap-batch1-state.json && \
  git diff --cached --name-only
# staged 5 个文件：sidebarConfig.js + AppSidebar.vue（data-module 视图改动）+ 2 test files（扩展 + 新建）+ state.json（completed 状态）
# 注意：AppSidebar.test.js 不改（R2-F003 新独立文件承接 conduct 入口级）
```
...
- Modify: `/home/ops/projects/edu-cloud/CLAUDE.md`（进行中 → 已完成设计）
...
cd /home/ops/projects/edu-cloud && python -m pytest tests/test_conduct/ -q --tb=no 2>&1 | tail -3
```
Expected: `80 passed`（68 ECS 基线 + 3 T4 governance + 4 T1 helper + 1 T1 入口级 API 403 + 1 T1 对照组 subject_teacher 200（R5-F001）+ 3 T2 入口级 = 80）。

Run:
```bash
cd /home/ops/projects/edu-cloud && python -m pytest tests/test_services/test_school_settings_service.py tests/test_services/test_homework_permissions.py -q 2>&1 | tail -3
```
Expected: `15 passed`。

Run:
```bash
cd /home/ops/projects/edu-cloud/frontend && npx vitest run src/__tests__/sidebarConfig.conduct.test.js src/__tests__/AppSidebar.test.js src/__tests__/AppSidebar.conduct.test.js src/pages/parent/__tests__/ParentRules.spec.js src/__tests__/permissions.lesson_prep.test.js 2>&1 | tail -5
```
Expected: `29 passed`（13 基线 3 件套 + 10 T3 sidebarConfig 新增（9 矩阵 + 1 F007 perm 合法性）+ 1 T3 AppSidebar.conduct.test.js F006 入口级 + 5 T1 permissions.lesson_prep = 29；AppSidebar.test.js 原 3 保持）。conduct 后端套件配套 `80 passed`。
...
2. `test_baseline.conduct_backend`：`68` → `80`
...
Run:
```bash
cd /home/ops/projects/edu-cloud && git log --oneline --grep="T[1-5]" -10
```
...
> [2026-MM-DD HH:MM:SS 批次 1 实现完成] Commits: <first>..<last> | Tests: conduct=80, services=15, frontend_conduct_suite=29
>
...
使用 Edit 工具，`/home/ops/projects/edu-cloud/CLAUDE.md`:
...
```markdown
- **德育板块统筹规划路线图 批次 1 [实现完成]**: R3 PASS 后的全景治理批次 1 — T5 post-T-Wipe 文档验证 / T4 conduct MODULE.md 补全 / T1 lesson_prep_leader 权限回收（C 方案，behavior_change R-T1）/ T2 AddPointsRequest.date→record_date（修 pydantic v2 field-type shadowing bug，behavior_change R-T2）/ T3 sidebar 按 permissions 派生（academic_director 入口 3→7，grade_leader 3→5，subject_teacher 2→4 per R-T3-followup，behavior_change R-T3）。5 Tasks / 7 commits / Gate 1 R2 PASS + Gate 2 PASS。测试分项: conduct 80 / services 15 / frontend conduct suite 29（ECS pytest 基线 68 + T1/T2/T4 共 12 增量）。批次 2 (D-005/D-009/D-010 运维就绪) 与批次 3 (D-006/D-007/D-008 真实验证) 占位等待启动。真源: `edu-cloud/docs/plans/2026-04-14-conduct-roadmap-design.md`
```
...
测试分项: conduct 80 / services 15 / frontend 29（ECS pytest 基线 68 + T1/T2/T4 共 12 增量）。
...
## R1/R2 Findings 处置总结（2026-04-14 Gate 1 R1 + 2026-04-17 R2/R3）

Gate 1 **R1** 结论: FAIL（9 finding，HIGH×4 / MED×5，raw log SHA256 `9554e5ee...`）。R2 修订针对每项真正回填到主 Task 正文（非附录声明）。

Gate 1 **R2** 结论: FAIL（6 finding，HIGH×5 / MED×1，raw log SHA256 `90e2f2a9...`，plan hash `97894724...`）。R3 修订聚焦：
- R2-F001: Task 3 Step 3.4a import 路径 `student/models` → `edu_cloud/models/user`+`user_role`，复用 conftest pattern（set_password/create_access_token sub+role）
- R2-F002: Task 4 Step 4.2 文件名 `test_admin_api.py` → `test_admin_crud_api.py`；3 条测试契约命令统一；反例 `assert data[0]["date"]` → `assert str(rec.date)` DB readback 口径
- R2-F003: Step 5.2a 分 Part A（顶部 import 插入）+ Part B（describe 末尾追加）；Step 5.2b 新建 `AppSidebar.conduct.test.js` 独立 harness；Task 5 Files 加 `AppSidebar.vue`（视图层 data-module）+ 新 test 文件
- R2-F004: T2 范围收窄为后端 API 契约修复，不改前端；Files 移除 frontend 两文件；Step 4.1 审计只限后端；Step 4.7/4.8 改为"不改前端"；审查清单/关键行为收窄
- R2-F005: design.md §3.4 L238-240 改为"已批准 F005"；§5.1 L342-345 改为"已批准落地"
- R2-F006: Contract Pack INV-T1-003 收窄为 API 403 单语义；ToolAccessResolver AI tool 过滤拆到 TD-006 批次 3 独立验证

| ID | Sev | Cat | Type | R1 状态 | R2 状态 | R3 处置位置 |
|---|---|---|---|---|---|---|
| F001/R1 | HIGH | code-bug | defect_fix | resolved-inline | verified | Task 4 admin_router.py:115,137 （R2 保持）|
| F002/R1 | HIGH | test-gap | defect_fix | resolved-inline | verified | Task 4 Step 4.2 POST 200 + DB readback（R2 保持）|
| F003/R1 | MED | code-bug | defect_fix | resolved-inline | verified | Step 2.7 scripts/governance/（R2 保持）|
| F004/R1 | MED | code-bug | defect_fix | resolved-inline | verified | Task 1-5 state.json N.0/N.final Steps（R2 保持）|
| F005/R1 | HIGH | design-concern | behavior_change | approved | verified | F005 用户 2026-04-14T07:45 批准；R3 清理 design §3.4/§5.1 残留 pending |
| F006/R1 | MED | test-gap | defect_fix | resolved-inline | **R2-F001/F003 contested** | R3 重写 Step 3.4a import 路径 + 新建 AppSidebar.conduct.test.js 独立 harness |
| F007/R1 | MED | test-gap | defect_fix | resolved-inline | **R2-F003 contested** | R3 Step 5.2a 分 Part A/B；Files 加 AppSidebar.vue 视图层改动 |
| F008/R1 | HIGH | design-concern | defect_fix | resolved-inline | **R2-F006 partial** | R3 INV-T1-003 收窄单语义；AI tool 过滤拆 TD-006 |
| F009/R1 | MED | code-bug | defect_fix | resolved-in-design | verified | design §8 / plan Task 6 Step 6.4 已对齐（R2 保持）|
| R2-F001 | HIGH | code-bug | defect_fix | — | NEW | Task 3 Step 3.4a import 路径修复 + 复用 conftest pattern |
| R2-F002 | HIGH | code-bug | defect_fix | — | NEW | Task 4 Step 4.2 / 契约命令 / 反例 清理 test_admin_api.py 旧口径 |
| R2-F003 | HIGH | code-bug | defect_fix | — | NEW | Task 5 Step 5.2a/5.2b harness 改造 + AppSidebar.vue data-module 视图改动 |
| R2-F004 | HIGH | code-bug | defect_fix | — | NEW | T2 范围收窄后端 API 契约修复 |
| R2-F005 | HIGH | design-concern | defect_fix | — | NEW | design.md §3.4 + §5.1 清理 pending 表述 |
| R2-F006 | MED | code-bug | defect_fix | — | NEW | Contract Pack INV-T1-003 收窄 + TD-006 新增 |

R3 修订总原则：不再通过"附录声明"替代主正文落地；每处修改都要指向现有 test harness / code path，Executor 可直接落地运行。
...
contract_pack:
  invariants:
    ...
    - id: INV-T1-003
      statement: "lesson_prep_leader 调 conduct API 入口被拒（HTTP 403）"
      rationale: "权限回收需在 API 守卫层面生效（R1-F006 入口级验证）。R2-F006 收窄：原 prose 混装 ToolAccessResolver AI tool 过滤，拆到 TD-006 批次 3 独立验证"
      verification: pending_test
  ...
  test_debt:
    ...
    - id: TD-005
      ...
      deadline: "2026-12-31"
    - id: TD-006
      item: "ToolAccessResolver 对 lesson_prep_leader 过滤 6 conduct AI tools 的入口级测试（确认 Agent 工具链不越权）"
      reason: "R2-F006 拆分出来的独立语义；归批次 3 D-007（Agent happy-path 真实验证）；批次 1 仅以 RBAC 红测 + API 403 兜底，不验 Agent 工具层"
      deadline: "2026-05-15"
...
Plan saved to `/home/ops/projects/edu-cloud/docs/plans/2026-04-14-conduct-roadmap-batch1-plan.md`.
