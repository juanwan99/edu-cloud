...
- Move: `src/edu_cloud/services/workspace_service.py` → `src/edu_cloud/modules/exam/workspace_service.py`
- Move: `src/edu_cloud/services/school_service.py` → `src/edu_cloud/modules/school/service.py`
- Move: `src/edu_cloud/models/ai_session.py` → `src/edu_cloud/ai/models.py`
- Delete: `src/edu_cloud/api/sync_students.py`（依赖已删除的 ExamResult 旧语义 + ClassGroup，且 sync 机制废弃）
...
- [ ] **Step 5: 移动 Paper + Workspace + School service + AI session model**

School: `services/school_service.py` → `modules/school/service.py`（留 re-export stub）
AI Session: `models/ai_session.py` → `ai/models.py`（留 re-export stub）
删除 `api/sync_students.py`（sync 废弃，依赖旧模型）
