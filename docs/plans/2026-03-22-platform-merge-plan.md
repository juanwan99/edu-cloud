...
│   │   ├── service.py             # 迁入 grading/ services
...
- Create: `src/edu_cloud/modules/{exam,student,card,scan,grading,marking,analytics,bank,knowledge,profile,pipeline,studio,calendar,paper}/__init__.py`
...
for mod in exam student card scan grading marking analytics bank knowledge profile pipeline studio calendar paper; do
...
Expected: 15 `__init__.py` files (14 modules + modules root)
...
- Create: `src/edu_cloud/modules/exam/llm_router.py` — 迁入 slot 选择逻辑
...
从 `exam-ai/src/exam_ai/services/llm_router.py` 复制到 `src/edu_cloud/modules/exam/llm_router.py`。
更新 import 路径。

- [ ] **Step 4: 写 LLMSlot 基础测试**

```python
# tests/test_models/test_llm_slot.py
async def test_llm_slot_creation(db):
    slot = LLMSlot(slot_number=1, api_url="http://test", api_key="k", model="m")
    db.add(slot)
    await db.commit()
...
- Delete: `src/edu_cloud/models/exam.py`（edu-cloud 原有的精简 Exam，被 exam-ai 完整版替代）
- Test: 写 Exam/Subject/Question 基础 CRUD 测试

- [ ] **Step 1: 迁入 Exam/Subject/Question 模型**

从 exam-ai `models/exam.py` 复制 Exam, Subject 类定义。从 `models/grading.py` 中提取 Question（如果 Question 在那里）。添加到 `modules/exam/models.py`。
...
- 删除 edu-cloud 原有的 `models/exam.py`
...
- [ ] **Step 3: 移动已有 services**

`services/joint_exam_service.py` → `modules/exam/joint_exam_service.py`
`services/results_service.py` → `modules/exam/results_service.py`
更新 import 路径。
...
- Delete: `src/edu_cloud/models/student.py`（旧版）
- Delete: `src/edu_cloud/models/class_group.py`
- Modify: 所有引用 ClassGroup 的文件 → Class
...
Template + CardSkeleton → `modules/card/models.py`。CardSkeleton 加 school_id FK。
...
### Task 11: Knowledge 模块合并 + Studio/Calendar 重组
...
- Move: `src/edu_cloud/knowledge/store.py` → `src/edu_cloud/modules/knowledge/store.py`
- Move: `src/edu_cloud/knowledge/loader.py` → `src/edu_cloud/modules/knowledge/loader.py`
- Move: `src/edu_cloud/services/studio_service.py` → `src/edu_cloud/modules/studio/service.py`
- Move: `src/edu_cloud/services/approval_service.py` → `src/edu_cloud/modules/studio/approval_service.py`
- Move: `src/edu_cloud/models/document.py` → 内容合入 `modules/studio/models.py`
- Move: `src/edu_cloud/models/approval.py` → 内容合入 `modules/studio/models.py`
- Move: `src/edu_cloud/services/calendar_service.py` → `src/edu_cloud/modules/calendar/service.py`
- Move: `src/edu_cloud/services/notification_service.py` → `src/edu_cloud/modules/calendar/notification_service.py`
- Move: `src/edu_cloud/models/calendar.py` + `notification.py` → 合入 `modules/calendar/models.py`
- Move: `src/edu_cloud/services/paper_service.py` → `src/edu_cloud/modules/paper/service.py`
- Move: `src/edu_cloud/services/workspace_service.py` → `src/edu_cloud/modules/exam/workspace_service.py`（或独立模块）
...
将 models + services 移入 `modules/studio/`。更新 import。

- [ ] **Step 4: 重组 Calendar 模块**

将 models + services 移入 `modules/calendar/`。更新 import。

- [ ] **Step 5: 移动 Paper + Workspace service**

- [ ] **Step 6: 清理旧 services/ 和 models/ 目录**

删除已移入模块的旧文件。保留 `services/exceptions.py`（移到 `core/exceptions.py`）。

- [ ] **Step 7: 运行全量测试（大量 import 变更，预计需要修复）**

```bash
python -m pytest --tb=short -q
```

这一步预计会有大量 ImportError 需要修复。逐个修复直到全部 PASS。

- [ ] **Step 8: Commit**

```bash
git add -A && git commit -m "refactor: 重组 knowledge/studio/calendar/paper 到模块结构"
```
...
- Modify: `src/edu_cloud/core/permissions.py` — 更新 ROLE_TOOL_CATEGORIES
...
`permissions.py` 中 `ROLE_TOOL_CATEGORIES` 更新为 9 类别，按设计文档 §5.2 映射。
...
- [ ] **Step 1: 生成新 Alembic migration**

```bash
cd C:/Users/Administrator/edu-cloud
# 确保所有模型都被 env.py import
python -m alembic revision --autogenerate -m "merge_exam_ai_into_edu_cloud"
```
