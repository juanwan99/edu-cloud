...
可与 A 并行启动。A 产出 S1-A migration slug 后，B 的 migration down_revision placeholder 要替换为真实 slug。
...
第二步：用 superpowers:writing-plans。linear chain 第 2 环 down_revision='<待 S1-A Task 2 实施后确认 slug>' placeholder，实施前必须跑 alembic heads 替换为真实值。
