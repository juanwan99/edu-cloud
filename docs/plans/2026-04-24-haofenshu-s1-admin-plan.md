...
baseline_count: 2079 passed / 22 failed / 1 error / 23 skipped (post-S1-A @ commit 86b4ca5)
...
> **⚠️ Baseline 诚实披露**（L015 反虚假完成）：当前 HEAD（`c155ab5` conduct 批次 1 收尾）baseline 含 **22 failed + 1 error**（pre-S1-A 既有技术债，S1-A 闭环 @ `86b4ca5` 后保持不变，见 S1-A plan §Deferred 第 7 条）。S1-C Gate 只要求"不新增 failure + 新测试全绿"，不承担既有技术债处置。
...
**Q2_excluded**:
- "grades.id 用 Integer（顺应 S1-A bank_questions.grade_id 的 Integer）": 反证路径: 违反 IdMixin 约定（所有 88 张表的 id 都是 String(36) UUID），引入 schools.id (String) / grades.id (Integer) / users.id (String) 的混合类型，破坏项目一致性。grep 现有表证明全项目零 Integer primary key：
  ```bash
  grep -l "Integer, primary_key" src/edu_cloud/modules/*/models.py src/edu_cloud/models/*.py
  ```
  零匹配。
...
**decision**: Grade 放 `src/edu_cloud/models/grade.py`（跨模块共享表，`orm-placement.md:§7` 反模式"跨模块共享表下沉到某模块"）。TeachingPlan 放 `src/edu_cloud/modules/calendar/models.py`（模块专用，对应 design §6.3 `calendar.teaching_plan_service`）。**注册方式**：`alembic/env.py` + `src/edu_cloud/api/app.py` 各加 1 行 import；`src/edu_cloud/models/__init__.py` **不**参与（已验证空文件）。
...
- **How to apply**: Task 5 smoke test `test_migration_file_down_revision_is_s1a_slug` 字符串匹配断言；Task 4 `alembic heads` 三阶段 smoke（stamp→upgrade→downgrade→upgrade）每步确认单 head。
...
- **statement**: S1-C migration 文件 head 处 `down_revision` 字符串字面值必须等于 S1-A T2 slug（当前实测 `'a88094ee4ea6'`），严禁其他历史 head 或 base revision（待 Task 5 落地 `tests/test_alembic_s1c_admin.py::test_migration_file_down_revision_is_s1a_slug`）
...
### ORC-S1C-005: ORM 注册走 env.py + app.py，零 __init__.py 依赖

- **Rule**: S1-C 新增 Grade 模型时必须在 `alembic/env.py` 和 `src/edu_cloud/api/app.py` 各加 1 行 import；禁止新增 `src/edu_cloud/models/__init__.py` 的 import 语句（该文件验证为空，不参与注册）。TeachingPlan 复用已存在的 `import edu_cloud.modules.calendar.models` 无需新增行。
- **Why**: F002 parent review 根因（E-003）。`models/__init__.py` 空文件不被引用；Alembic autogenerate 和 `Base.metadata.create_all()` 都依赖 env.py + app.py 两处 import。
- **How to apply**: Task 1 最终 diff 应含 `alembic/env.py` 新增一行 `from edu_cloud.models.grade import Grade  # noqa: F401`；Task 1 最终 diff 应含 `src/edu_cloud/api/app.py` 新增一行 `import edu_cloud.models.grade  # noqa: F401`；禁止修改 `src/edu_cloud/models/__init__.py`。
- **Violation reporter**: Task 5 smoke `git diff --stat $S1C_BASE..HEAD src/edu_cloud/models/__init__.py` 返回空；Task 1 Step 1.7 `grep "from edu_cloud.models.grade" alembic/env.py src/edu_cloud/api/app.py` 两处各返回 1 行。
- **type**: forbidden_strategy
- **protects**: [state_machine]
- **verification**: pending_test
- **statement**: S1-C 出口处 `src/edu_cloud/models/__init__.py` 零改动；`alembic/env.py` + `src/edu_cloud/api/app.py` 各新增恰好 1 行 Grade import（待 Task 5 落地 `tests/test_alembic_s1c_admin.py::test_orm_registration_in_env_and_app_only`）
...
contract_pack:
  ...
  invariants:
    ...
    - id: INV-S1C-005
      statement: "PaperAccessLevel 枚举严格有 3 个成员值 {teacher_private, school_shared, district_shared}，且实例是 str+Enum 可参与字符串比较（非逻辑镜像断言，通过 round-trip 构造/序列化/反序列化验证，参见 F006 修正）（待 Task 5 落地 tests/modules/paper/test_access_level.py::test_paper_access_level_roundtrip_via_value）"
      verification: pending_test
    ...
    - id: INV-S1C-007
      statement: "S1-C upgrade 后 alembic heads subprocess 输出过滤空行后恰好 1 行；downgrade -1 后回到 S1-A slug 'a88094ee4ea6'（待 Task 5 落地 tests/test_alembic_s1c_admin.py::test_migration_chain_head_is_single + test_downgrade_restores_s1a_head）"
      verification: pending_test

    - id: INV-S1C-008
      statement: "S1-C 出口处 src/edu_cloud/models/__init__.py 字节级一致（未修改）；alembic/env.py 与 src/edu_cloud/api/app.py 各新增恰好 1 行 Grade import（待 Task 5 落地 tests/test_alembic_s1c_admin.py::test_orm_registration_in_env_and_app_only）"
      verification: pending_test
...
| `tests/models/test_grade.py` | **新建** | Grade ORM roundtrip（import / fields / FK to schools / UniqueConstraint） |
| `tests/modules/student/test_class_grade_id.py` | **新建** | Class.grade_id 新增字段 + 守旧字段不动验证 |
| `tests/modules/calendar/test_teaching_plan_model.py` | **新建** | TeachingPlan ORM roundtrip（import / fields / 3 FK targets 限定） |
| `tests/modules/paper/test_access_level.py` | **新建** | PaperAccessLevel 枚举 3 成员 + round-trip via value（F006 反逻辑镜像） |
...
- Test: `tests/models/test_grade.py`（new）+ `tests/modules/student/test_class_grade_id.py`（new）
...
- **反例**:
  - 若 Task 1 漏 import `Grade` 到 `alembic/env.py` / `app.py`：Alembic autogenerate 发现不到 Grade → `test_grade_registered_in_env_and_app` 断言失败（grep 命令输出匹配校验）
...
- **命令**:
  ```bash
  .venv/bin/python -m pytest tests/models/test_grade.py tests/modules/student/test_class_grade_id.py -v --tb=short
  ```
...
Create `tests/modules/student/test_class_grade_id.py`:
...
- [ ] **Step 1.7: 跑测试确认 PASS + ORM 注册断言**

```bash
cd /home/ops/projects/edu-cloud
.venv/bin/python -m pytest tests/models/test_grade.py tests/modules/student/test_class_grade_id.py -v --tb=short

# ORC-S1C-005 手动守卫：两处 import 都在
...
```bash
git add src/edu_cloud/models/grade.py src/edu_cloud/modules/student/models.py alembic/env.py src/edu_cloud/api/app.py tests/models/test_grade.py tests/modules/student/test_class_grade_id.py
git commit -m "$(cat <<'EOF'
feat(models): S1-C T1 Grade 独立表 + Class.grade_id FK + ORM 注册
...
- alembic/env.py + api/app.py 各追加 1 行 Grade import（ORC-S1C-005）
...
- **回归**: 现有 `tests/test_modules/test_calendar/*` 继续绿；CalendarEvent/NotificationRule/Notification 三个 re-export 仍可 import
- **命令**:
  ```bash
  .venv/bin/python -m pytest tests/modules/calendar/test_teaching_plan_model.py tests/test_modules/test_calendar/ -v --tb=short
  ```
...
```bash
cd /home/ops/projects/edu-cloud
.venv/bin/python -m pytest tests/modules/calendar/test_teaching_plan_model.py -v --tb=short
```
...
```bash
cd /home/ops/projects/edu-cloud
.venv/bin/python -m pytest tests/modules/calendar/test_teaching_plan_model.py tests/test_modules/test_calendar/ -v --tb=short 2>&1 | tail -30
```
...
```bash
git add src/edu_cloud/modules/calendar/models.py tests/modules/calendar/test_teaching_plan_model.py
git commit -m "$(cat <<'EOF'
...
- **命令**:
  ```bash
  .venv/bin/python -m pytest tests/modules/paper/test_access_level.py tests/test_services_exam/test_bank_service.py -v --tb=short
  ```
...
```bash
cd /home/ops/projects/edu-cloud
.venv/bin/python -m pytest tests/modules/paper/test_access_level.py -v --tb=short
```
...
```bash
cd /home/ops/projects/edu-cloud
.venv/bin/python -m pytest tests/modules/paper/test_access_level.py tests/test_services_exam/test_bank_service.py -v --tb=short 2>&1 | tail -40
```
...
```bash
git add src/edu_cloud/modules/paper/constants.py src/edu_cloud/modules/bank/models.py tests/modules/paper/test_access_level.py tests/test_services_exam/test_bank_service.py
git commit -m "$(cat <<'EOF'
...
- **命令**:
  ```bash
  .venv/bin/alembic heads  # 核实 head 是 'a88094ee4ea6'
  .venv/bin/alembic revision -m "s1c_admin_schema"
  # 手动填 upgrade/downgrade body
  .venv/bin/alembic stamp a88094ee4ea6
...
Expected: 三阶段全部成功，每阶段 `alembic heads` 返回单 head；无 SQL 语法错误；无 NOT NULL violation。
...
- **反例**:
  ...
  - 若 Task 1 改了 `models/__init__.py`：`test_orm_registration_in_env_and_app_only` git diff 检测 fail
- **边界**:
...
def test_migration_file_exists_and_down_revision_matches_prev_head():
    """INV-S1C-006: S1-C migration 文件 head 处 down_revision 是 S1-A slug 'a88094ee4ea6'.
...
def test_orm_registration_in_env_and_app_only():
    """ORC-S1C-005: alembic/env.py + api/app.py 各含 Grade import；__init__.py 零改动."""
    # alembic/env.py 含 Grade import
    ...
    assert "from edu_cloud.models.grade import Grade" in env_content, \
        "alembic/env.py 缺 Grade import"

    # api/app.py 含 Grade import
    ...
    assert "import edu_cloud.models.grade" in app_content, \
        "api/app.py 缺 Grade import"

    # models/__init__.py 仍是空文件（0 非空行）
...
def test_migration_chain_head_is_single(sqlite_db):
    """INV-S1C-007: upgrade 后 alembic heads 仍返回 1 行."""
...
def test_downgrade_restores_s1a_head(sqlite_db):
    """INV-S1C-007: downgrade -1 后 head 回到 'a88094ee4ea6'."""
    r = _run_alembic(['upgrade', 'head'], sqlite_db)
    assert r.returncode == 0
    ...
    r = _run_alembic(['heads'], sqlite_db)
    assert r.returncode == 0
    head = r.stdout.splitlines()[0].strip().split()[0]
    assert head == 'a88094ee4ea6', f"Expected a88094ee4ea6, got {head}"


def test_grades_table_created_with_expected_schema(sqlite_db):
    """INV-S1C-001: upgrade 后 grades 表存在且列完整."""
    r = _run_alembic(['upgrade', 'head'], sqlite_db)
    assert r.returncode == 0
...
def test_teaching_plans_table_and_fks_ok(sqlite_db):
    """INV-S1C-002 + ORC-S1C-003: teaching_plans 表 + FK 目标 ⊂ {schools, grades, users}."""
    r = _run_alembic(['upgrade', 'head'], sqlite_db)
    assert r.returncode == 0
...
- S1-C 新增测试全绿（Task 1 的 8 + Task 2 的 5 + Task 3 的 7 + Task 5 的 9 = **29 个新 test**）
...
9 个 test 机械化 5 条 ORC：
- ORC-S1C-001: test_migration_file_down_revision_matches_prev_head / test_migration_chain_head_is_single / test_downgrade_restores_s1a_head
- ORC-S1C-002: test_classes_grade_id_added_legacy_unchanged（守旧字段精确长度/nullability 断言）
- ORC-S1C-003: test_teaching_plans_table_and_fks_ok（FK target 集合断言 ⊂ 3 张表）
- ORC-S1C-004: test_bank_questions_grade_id_is_string36_with_fk / test_all_grade_id_fks_are_string36
- ORC-S1C-005: test_orm_registration_in_env_and_app_only（__init__.py 零内容 + 2 处 import 命中）
- INV-S1C-001/002: test_grades_table_created_with_expected_schema / teaching_plans

S1-C Gate G1 证据：全量测试 22 failed + 1 error 保持（不新增），新加 29 test 全绿。
...
3. **全量测试**: 22 failed + 1 error 保持（不新增），S1-C 29 个新 test 全绿
4. **ORM 注册断言**: `alembic/env.py` + `src/edu_cloud/api/app.py` 各新增 1 行 Grade import；`src/edu_cloud/models/__init__.py` 零改动
...
4. **ORC 对齐**:
   ...
   - ORC-S1C-005 ORM 注册路径 → Task 1 Step 1.5/1.6 加行 + Task 5 Step 5.1 `test_orm_registration_in_env_and_app_only`

5. **Evidence Block 真实性（F008 传承）**:
   - E-001 真实 `alembic heads` 实测输出（2026-04-24T20:12）+ 负面 grep 命令 + 完整 chain
   - E-002 真实 `src/edu_cloud/models/base.py:11-14` IdMixin 源码引用 + 负面 grep (零 Integer PK)
...
**修正状态**: 本次 self-review 未发现需要回改的问题。
