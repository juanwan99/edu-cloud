...
| Session | 本次对话消息历史 + 工具调用结果 | 会话级（TTL 2h） | Redis（可用时）/ 内存 fallback（单实例） |
| Working | 中间计算结果缓存 | 步骤级 | 内存 |

**Session 存储策略**：优先使用 Redis（exam-ai 已配置 `REDIS_URL` 用于 arq）。学校端部署如果没有 Redis，fallback 到进程内 dict（单实例部署足够，重启丢失对话历史可接受）。
...
- 映射表与会话绑定，会话结束即销毁
- 不持久化，不落盘
- **业务学号**（student_number，如"20240101"）永远不发送给 LLM
- **系统 UUID**（Student.id）可以进入 LLM 上下文作为工具参数（不含个人信息）
- **班级名称**无需脱敏（非个人信息）
- **SSE 流式输出**：反脱敏在 AgentEvent 的 answer 类型中做整句 buffer，不做逐 token 替换（避免代号被截断）
...
| 速率限制 | Phase 1 实现：10 次/分钟/用户，200 次/天/用户，单次会话最多 20 次 API 调用 |
| 成本控制 | Phase 2 实现：Token 配额（学校 200 万/月，用户 5 万/天），Redis 计数器，超额降级到轻量模型。Phase 1 仅做速率限制 |
