...
- Modify: `src/edu_cloud/api/module_middleware.py`（ROUTE_MODULE_MAP 添加 homework）
...
@pytest.mark.asyncio
async def test_scope_subject_teacher_only_own(client, hw_api_fixtures, db):
    """subject_teacher 只能看到自己布置的作业（F-03 ScopeFilter）。"""
    from edu_cloud.models.user import User
    from edu_cloud.models.user_role import UserRole

    # 创建另一个科任教师
    other = User(username="hw_other_teacher", display_name="另一老师")
    other.set_password("123456")
    db.add(other)
    await db.flush()
    db.add(UserRole(
        user_id=other.id, role="subject_teacher",
        school_id=hw_api_fixtures["school_id"], is_primary=True,
    ))
    await db.commit()
    ...
    from edu_cloud.modules.homework.service import HomeworkTaskService
    await HomeworkTaskService.create_task(
        db, school_id=hw_api_fixtures["school_id"], title="别人的作业",
        task_type="regular", subject_code="SX",
        assigned_by=other.id,
    )
    await db.commit()

    # 用 hw_teacher（homeroom）查列表能看到
    resp = await client.get("/api/v1/homework/tasks", headers=await _get_headers(client, "hw_api_teacher"))
    # homeroom_teacher 能看到全部本校的（非 subject_teacher）
    assert len(resp.json()) >= 1
...
class TaskCreate(BaseModel):
    ...
    deadline: datetime | None = None  # F-06: 与 service 签名一致
    content: str | None = None
...
- [ ] **Step 3: 在 app.py 注册 router + 导入 model + 更新 module_middleware**
...
在 `src/edu_cloud/api/module_middleware.py` 的 `ROUTE_MODULE_MAP` 中添加：
```python
...
**测试契约:**
1. observer 无权限 → 403
   - 入口: `POST /api/v1/homework/tasks` with observer JWT
   - 反例: 错误实现缺少 require_permission → 200 放行
   - 边界: observer / parent(VIEW only) / grade_leader(VIEW only)
   - 回归: N/A
   - 命令: `pytest tests/test_api/test_homework_api.py::test_permission_denied -v`
2. 发布后自动创建 submissions（API 入口级）
   - 入口: `POST /api/v1/homework/tasks/{id}/publish`
   - 反例: 错误实现只改状态不创建 submissions → GET submissions 返回空
   - 边界: 0 学生 / 3 学生 / class_id 为空
   - 回归: N/A
   - 命令: `pytest tests/test_api/test_homework_api.py::test_publish_and_submissions -v`
3. 模块禁用时 403
   - 入口: `GET /api/v1/homework/tasks` with homework 模块禁用
   - 反例: 错误实现未在 ROUTE_MODULE_MAP 注册 → 200 放行
   - 边界: 模块启用 / 模块禁用 / 模块不存在
   - 回归: F-02
   - 命令: `pytest tests/test_api/test_homework_api.py::test_module_disabled_403 -v`

**边界条件:**
- observer 角色请求 → 403（无 VIEW_HOMEWORK）
- 空 title 创建 → 422 Pydantic 校验
- 无效 deadline 格式 → 422
...
- ✓ observer 角色收到 403
- ✓ module_middleware ROUTE_MODULE_MAP 包含 homework（F-02）
- ✓ ScopeFilter：subject_teacher 限 assigned_by（F-03）
...
@tools.register(
    ...
    domain="homework",
    risk_level="low",
    requires_capabilities={"homework.read"},  # F-05: 接入 capability 控制
...
**测试契约:**
1. 5 个工具注册到 registry
   - 入口: `tools.get_all()` 检查注册名
   - 反例: 错误实现忘记在 `__init__.py` 导入 → 工具不存在
   - 边界: 工具名拼写错误 / 重复注册
   - 回归: N/A
   - 命令: `pytest tests/test_ai/test_homework_tools.py::test_homework_tools_registered -v`
2. requires_capabilities 已填（F-05/F-08）
   - 入口: `tools.get_by_name("list_homework_tasks").requires_capabilities`
   - 反例: 错误实现缺少 requires_capabilities → ToolAccessResolver 不过滤
   - 边界: read 工具 / write 工具
   - 回归: F-05
   - 命令: `pytest tests/test_ai/test_homework_tools.py::test_homework_tools_capabilities -v`

**边界条件:**
- list 无数据 → 返回 `{"tasks": []}`
- recommend_remedial 任意 exam_id → 返回 stub 消息
- assign_homework 无学生班级 → 创建成功但 0 submissions

**审查清单:**
- ✓ 5 个工具全部注册到 registry
- ✓ module_code="homework" 确保模块开关生效
- ✓ domain="homework" 确保 IntentResolver 正确路由
- ✓ requires_capabilities 已填（F-05）
