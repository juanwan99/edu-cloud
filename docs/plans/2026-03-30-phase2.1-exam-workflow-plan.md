...
@pytest.mark.asyncio
async def test_completed_to_published(db):
    exam = Exam(name="Test", status="completed", school_id="s1")
    db.add(exam)
    await db.flush()
    result = await update_exam(db, exam_id=exam.id, school_id="s1", status="published")
    assert result.status == "published"


@pytest.mark.asyncio
async def test_published_to_archived(db):
    exam = Exam(name="Test", status="published", school_id="s1")
    db.add(exam)
    await db.flush()
    result = await update_exam(db, exam_id=exam.id, school_id="s1", status="archived")
...
Expected: test_completed_to_published FAIL（ValidationError 无效转换）

- [ ] **Step 3: Extend _VALID_STATUS_TRANSITIONS**

在 `src/edu_cloud/modules/exam/service.py` 中修改：
```python
_VALID_STATUS_TRANSITIONS = {
    ...
    "grading": {"reviewing"},
    "reviewing": {"completed"},
    "completed": {"published"},     # 新增
    "published": {"archived"},      # 新增
}
```
...
**审查清单:**
- ✓ completed→published 和 published→archived 新增
- ✓ archived 是终态（无出口）
- ✓ 不能跳过 published 直接归档
- ✓ 原有 5 个转换不受影响
- ✗ completed→published 尚无前置条件检查（Task 7 ExamPublishService 实现）
...
1. 新状态转换有效性
   ...
   - 反例: 未添加新转换会导致 ValidationError — 本测试捕获此错误
   - 边界: 所有 7 个状态间的无效转换组合
...
class GradingAssignmentService:
    ...
    async def assign_block(
        ...
        assignment = GradingAssignment(
            ...
            total_count=0, school_id=school_id,
        )
        db.add(assignment)
        await db.flush()
        return assignment
...
async def test_archive_success(db):
    ...
    await ExamPublishService.archive(db, exam_id=exam.id)
    await db.refresh(exam)
...
class ExamPublishService:
    ...
    async def publish(db: AsyncSession, *, exam_id: str, school_id: str) -> dict:
        ...
        if not exam or exam.status != "completed":
            raise StateError(
                f"Cannot publish exam in status '{exam.status if exam else 'not found'}'"
            )
    ...
    @staticmethod
    async def archive(db: AsyncSession, *, exam_id: str) -> None:
        exam = await db.get(Exam, exam_id)
        if not exam or exam.status != "published":
            raise StateError(
                f"Cannot archive exam in status '{exam.status if exam else 'not found'}'"
            )
        exam.status = "archived"
        logger.info("exam_archived: exam_id=%s", exam_id)
```
...
1. 前置条件阻塞发布
   ...
   - 边界: 阅卷全完成+无质量问题/有未完成/有 high severity
   - 回归: N/A
...
- ✓ assign_grading_task 的 risk_level="med"（写操作）
