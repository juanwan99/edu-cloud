...
@pytest.mark.asyncio
async def test_update_progress_zero_total(db):
    """total_count=0 时直接 completed"""
    a = GradingAssignment(
...
async def test_get_progress(db):
    ...
    progress = await GradingAssignmentService.get_progress(db, "e1")
...
async def test_list_assignments(db):
    ...
    result = await GradingAssignmentService.list_assignments(db, exam_id="e1")
    assert len(result) == 2
```
...
class GradingAssignmentService:
    ...
    async def assign_block(
        ...
        # F-06: total_count 由调用方传入（答卷数），不硬编码 0
        assignment = GradingAssignment(
    ...
    async def list_assignments(
        db: AsyncSession, exam_id: str, *, assigned_to: str | None = None,
    ) -> list[GradingAssignment]:
    ...
    @staticmethod
    async def get_progress(db: AsyncSession, exam_id: str) -> dict:
        assignments = await GradingAssignmentService.list_assignments(db, exam_id)
...
Run: `cd C:/Users/Administrator/edu-cloud && python -m pytest tests/test_services/test_assignment_service.py -v`
Expected: 10 passed (4 model + 6 service)
...
async def test_get_quality_report(db):
    ...
    report = await QualityCheckService.get_quality_report(db, "e1")
...
async def test_get_quality_report_empty(db):
    report = await QualityCheckService.get_quality_report(db, "nonexistent")
...
class QualityCheckService:
    ...
    @staticmethod
    async def get_quality_report(db: AsyncSession, exam_id: str) -> dict:
        stmt = select(GradingQualityCheck).where(GradingQualityCheck.exam_id == exam_id)
        result = await db.execute(stmt)
...
async def get_grading_progress(exam_id: str, _db=None, _school_id: str = ""):
    return await GradingAssignmentService.get_progress(_db, exam_id)
...
async def get_quality_report(exam_id: str, _db=None, _school_id: str = ""):
    return await QualityCheckService.get_quality_report(_db, exam_id)
...
- ✗ 工具未做 school_id scope 过滤（依赖调用方注入）
