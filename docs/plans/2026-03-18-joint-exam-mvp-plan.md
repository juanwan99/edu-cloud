...
**1. 重构 request_logging 中间件** — `except Exception` 改为只处理非 Service 异常，Service 异常 re-raise 让 FastAPI 异常处理器接管：

```python
        except Exception as exc:
            # 让已注册的 Service 异常穿透到 FastAPI exception_handler
            from edu_cloud.services.exceptions import (
...
**2. 注册全局异常处理器** — add after `app = FastAPI(...)`:
...
- Modify: `src/edu_cloud/core/permissions.py` — 为 `district_admin` 添加 `MANAGE_SCHOOLS` 权限
...
- ✗ 非参与校提交 → PermissionDeniedError → 403
...
5. 新增 `GET /sync/templates/{exam_id}/{subject_code}`（返回 zip 包，内含 skeleton.json + template.pdf；用 `zipfile` 内存构建 → StreamingResponse）
...
- [ ] **Step 0: 调研 exam-ai AIGradingResult 数据结构**

在 exam-ai 中 Grep `AIGradingResult` 模型，确认字段结构与 `detail_scores` 格式的映射关系：
```bash
cd C:/Users/Administrator/exam-ai && grep -n "class AIGradingResult" src/ -r
...
- ✓ push_scores 从 AIGradingResult 组装 detail_scores
- ✗ 不修改现有 exam-ai 模型或路由
