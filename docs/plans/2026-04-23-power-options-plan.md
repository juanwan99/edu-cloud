...
@pytest.mark.asyncio
async def test_power_options_empty(client, admin_headers, seed_school):
    """无完成考试时返回空 grades 列表。"""
...
async def _seed_exam_data(db, school_id: str):
...
@pytest.mark.asyncio
async def test_power_options_tree_structure(client, admin_headers, seed_school, db):
    """多年级多班多科目：验证树结构正确 + all 节点存在。"""
    await _seed_exam_data(db, seed_school.id)

    resp = await client.get("/api/v1/analytics/power-options", headers=admin_headers)
    assert resp.status_code == 200
    result = resp.json()
...
@pytest.mark.asyncio
async def test_power_options_excludes_draft(client, admin_headers, seed_school, db):
...
async def test_power_options_rbac_subject_filter(client, db, seed_school):
    ...
    db.add(UserRole(
        ...
        school_id=seed_school.id, is_primary=True,
        subject_codes=["math"],
    ))
    await db.commit()
...
async def _seed_level_score_data(db, school_id: str):
    """10 个学生，分数从 100 到 91。"""
...
@pytest.mark.asyncio
async def test_level_score_basic(client, admin_headers, seed_school, db):
    """10 学生、4 等级：验证划分和赋分值。"""
    data = await _seed_level_score_data(db, seed_school.id)
...
@pytest.mark.asyncio
async def test_level_score_empty(client, admin_headers, seed_school):
    """不存在的考试返回 404。"""
    resp = await client.post(
        "/api/v1/analytics/level-score/convert",
        json={
...
async def convert_level_score(
    ...
    stmt = (
        ...
        .where(ExamResult.school_id == school_id)
        .order_by(ExamResult.total_score.desc())
    )
    if class_id:
...
Expected: 1976+ passed（基线 1970 + 新增 6）
...
```typescript
    // 替换 F007 stub — 真实 power-options 端点
...
git commit -m "feat(frontend-nuxt): PowerFilter 级联筛选组件 + ECharts 依赖"
```
...
| 等级赋分端点 | 不存在考试→404 | A 等级人数=前20% | 原始分降序(ORC-005) | `.venv/bin/python -m pytest tests/test_api/test_analytics_level_score.py -v` |
