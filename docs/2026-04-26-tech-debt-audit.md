...
## 十二、修复记录（2026-04-26 同日修复）

### Phase 0 — 安全修复

| ID | 状态 | commit | 说明 |
|----|------|--------|------|
| C-01 | **FIXED** | `07523e6` | 9 处硬编码密码→settings.SEED_DEFAULT_PASSWORD |
| GPT-01 | **FIXED** | `07523e6` | 与 C-01 合并修复，teacher_router 2 处 |
| C-03 | **SKIPPED** | — | .env 未被 git 追踪，无需操作 |
| C-05 | **FIXED** | `addd3b4` (paper-seg) | 默认端口 8000→9000 |
| GPT-02 | **SKIPPED (误报)** | — | middleware JWT 异常后已正确跳过模块检查，认证由下游 deps.py 处理 |
| H-03 | **FIXED (最小)** | `812d52a` | token 缺失时清理 auth_state；完整 httpOnly 迁移降级 |
| H-04 | **FIXED** | `0da3cdc` | ChatPanel v-html + DOMPurify.sanitize() |
| H-05 | **FIXED** | `0da3cdc` | card-editor panel.js + render.js innerHTML + DOMPurify |

### Phase 1 — 稳定性修复

| ID | 状态 | commit | 说明 |
|----|------|--------|------|
| C-02 | **FIXED** | `809f22d` | 移除 app.py create_all()，统一 Alembic |
| C-04 | **SKIPPED** | — | .db/日志未被 git 追踪 |
| C-06 | **FIXED** | `f00c3a6` (ace) | pyproject.toml +Pillow +numpy |
| H-06 | **FIXED** | `4a73a8d` | npm audit fix，5+2 HIGH→0+0 |
| H-07 | **SKIPPED (不存在)** | — | 已用 IN 批量查询，纯内存聚合 |
| H-08 | **FIXED** | `4296a02` | grading.py INFO→WARNING + exc_info |
| GPT-03 | **SKIPPED (已足够)** | — | 已有 logger.warning + exc_info=True |
| H-12 | **SKIPPED** | — | .venv 重建跳过（运行中风险） |

### Phase 2 — 架构修复

| ID | 状态 | commit | 说明 |
|----|------|--------|------|
| H-01 | **FIXED** | `1a4ff7f` | 3 God Module→7 文件，68 端点守恒 |
| H-09 | **FIXED** | `133b624` | manualChunks +marked-katex |
| H-10 | **FIXED** | `05f8bde` (ace) | SQLite connection-per-request + Depends |
| H-11 | **FIXED** | `d3633b3` | .github/workflows/test.yml |
| H-02 | **FIXED (Top 3)** | `778708d` `448da49` `543cfe8` | ExamDetailPage 1368→263 + AiGradingPage 838→732 + GradingDispatchPage 826→758，11 子组件 |

### 修复统计

- **实际修复**: 15 条（12 edu-cloud + 2 ace + 1 paper-seg）
- **有据跳过**: 6 条（未追踪×2 / 误报×1 / 已优化×1 / 已足够×1 / 风险×1）
- **基线守恒**: pytest 2219 passed / vitest 348 passed / npm audit 0 HIGH
- **总 commit**: 16（edu-cloud 12 + paper-seg 2 + ace 2）
