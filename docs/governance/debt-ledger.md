...
|---|---|---|
| 开放 HIGH 风险 | 2（R-H3 / R-H4；R-H5 已由 D-08C 证据关闭） | 0 |
...
- 处置路径：W4（C3 复验窗，read_only + 线上凭据）→ 设计者签发 → D-08D first cut
  → runtime/online closeout；GitHub exact-HEAD CI 单独留作 Phase B/R-H2。
...
- 状态：**D-08D runtime/online closed; GitHub exact-HEAD CI remains Phase B/R-H2 unknown**。
  ...
  定向 Vitest 47 passed。GitHub Actions 状态因本地 `gh` token invalid + connector
  仅返回空 legacy status/PR-run，尚未确认。
...
| R-M1 | MED | portal 5 端点零消费者、过滤无守卫 | **source/runtime/online closed by D-08D `e9a9d9da` + `332862a0`**；GitHub exact-HEAD CI 余项归 R-H2/Phase B |
