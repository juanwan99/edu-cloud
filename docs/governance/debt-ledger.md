...
| 跨模块依赖环 | 21（54 edges）（D-03A 收口 30/55→21/54） | 持续下降 + gate 防回弹 |
...
### D-03 跨模块耦合 54 edges / 21 cycles（R-H4）— HIGH · 结构
...
- 处置路径（剩余）：W5+ 拆环 burn-down 批次（独占窗串行，每窗 2–3 环，附依赖图 diff
  证据）；准备层在 `modular-arch-restore` worktree（Plan 1 边界 gate，见 D-09 Q3）。
- 状态：**open（burn-down 进行中）**——首批 D-03A 已收口，基线 54 edges / 21 cycles。
