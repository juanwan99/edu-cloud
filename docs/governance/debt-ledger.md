...
| AI tool 语义错挂 | D-04A 后 `exam` 46→16；30 个明确归属工具已归位，student/action/system 10 个待裁定 | 按 owner 归位 |
...
- **D-04A（2026-06-22，Codex 本地全授权小窗）**：按 `docs/governance/module-semantics.yaml`
  ...
  开关露出。
- 剩余裁定：`student` domain 4 个、`action` domain 中 `exam` 2 个、`system` domain 4 个仍挂
  `exam`。其中 `student` 在当前学校模块语义里是基础身份域（`architecture_to_module_code.student=null`），
  `action/system` 为横切能力；三者需单独裁定“免控 / 归入现有码 / 新开关码”，不纳入 D-04A。
- 状态：**open（D-04A 已收口，剩余 student/action/system 裁定与实现）**。
