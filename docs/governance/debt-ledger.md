...
| 指标 | 现值（2026-06-22） | 完工线 |
|---|---|---|
| 开放 HIGH 风险 | 0（R-H1 后半已由 `f18a507` worker runtime fingerprint + live truthline/guardian green 关闭；R-H2 exact-HEAD CI、R-H3 全量前端回归、R-H4/D-03、R-H5/D-08C 均已关闭） | 0 |
| 跨模块依赖环 | 0（0 edges）（D-03A 30/55→21/54、D-03B 21/54→13/53、D-03C 13/53→4/52、D-03D 4/52→0/51、D-03E 0/51→0/50；D-03F 0/50→0/50：occurrence 级，analytics 公共有效分读模型 + 3 处跨模块 import 上移共享层，边经其它文件保留；D-03G 0/50→0/49：analytics→studio 报告文档编排上移；D-03H 0/49→0/48：pipeline→bank 题库/错题本制品上移共享层；D-03I 0/48→0/41：pipeline 冷数据 owner 上移共享层，一次拆 7 条 pipeline→{exam,grading,knowledge,knowledge_tree,profile,scan,student} 边；D-03J 0/41→0/35：exam_import 导入写入链 owner 上移共享层，一次拆 6 条 exam_import→{exam,grading,pipeline,profile,scan,student} 边；D-03K 0/35→0/32：marking 阅卷工作流跨模块模型/解析助手访问上移共享层，一次拆 3 条 marking→{exam,grading,scan} 边；D-03L 0/32→0/29：scan 扫描工作流跨模块模型/常量访问上移共享层，一次拆 3 条 scan→{exam,card,student} 边；D-03M 0/29→0/26：grading 阅卷工作流跨模块模型/常量/工具访问上移共享层，一次拆 3 条 grading→{exam,card,scan} 边；D-03N 0/26→0/23：homework 考后补救工作流跨模块模型访问上移共享层，一次拆 3 条 homework→{exam,scan,bank} 边；D-03O 0/23→0/18：conduct 德育工作流跨模块模型/错题本服务访问上移共享层，一次拆 5 条 conduct→{academic,bank,exam,profile,student} 边；D-03P 0/18→0/11：bank/knowledge/portal/profile 小 owner 批次跨模块模型/服务访问上移共享层，一次拆 7 条边；D-03Q 0/11→0/7：academic/card/knowledge_tree 剩余小 owner 批次跨模块模型/服务访问上移共享层，一次拆 4 条边；D-03R 0/7→0/0：analytics 最后跨模块 owner 读取上移共享层，一次拆 7 条边） | 已清零 + gate 防回弹 |
| AI tool 语义错挂 | D-04B 后 `exam` 46→16→6；`study_analytics` 24、`research` 9、`studio` 2、`null/base` 8；组合依赖纳入基线 | closed/gate-built |
...
- **历史 review-gap（L2）= open**：06-07 后零 receipt 的 commit 残留，**从 W1 记录的
  ...
  `legacy receipt log` 末条仍 = 06-07 14:40 `PASS@3688f32`。
...
### D-03 跨模块耦合（R-H4，历史峰值 53 edges / 13 cycles → 已清零 0 edges / 0 cycles）— HIGH · 结构
...
- burn-down 记录：
  ...
  - **D-03Q（2026-06-22，Codex 本地全授权小窗）**：academic / card / knowledge_tree
    三个剩余小 owner 模块的跨模块模型/服务读取上移至模块外 facade：`services.academic_workflow`
    （calendar/student）、`services.card_workflow`（exam）、`services.knowledge_tree_workflow`（adaptive）——
    一次性消除 `academic -> {calendar, student}`、`card -> exam`、`knowledge_tree -> adaptive`
    共 4 条依赖边。基线 **11→7 edges、0 cycles 不变**。三个 MODULE.md
    `depends_on.modules` 清零并登记对应 workflow service；新增
    `tests/governance/test_d03q_remaining_small_boundaries.py` 固定直接 import 禁止与 facade re-export 不变量；
    academic 14 个测试、card 133 个测试、knowledge_tree/adaptive 202 个测试（10 skipped）、边界 2 个测试通过。
  - **D-03R（2026-06-22，Codex 本地全授权小窗）**：analytics 最后保留的跨模块 owner 读取
    （exam / grading / scan / student / knowledge / knowledge_tree / profile 模型与 grading AI helper）
    上移至模块外 facade `services.analytics_workflow`（纯 re-export facade，对外符号与 owner 模块同一对象）——
    analytics 不再直接 import 上述 7 个 owner 模块，一次性消除
    `analytics -> {exam, grading, knowledge, knowledge_tree, profile, scan, student}` 7 条依赖边。
    基线 **7→0 edges、0 cycles 不变**；analytics MODULE.md `depends_on.modules` 清零并登记
    `analytics_workflow`；新增 `tests/governance/test_d03r_analytics_boundary.py` 固定直接 import 禁止与
    facade re-export 不变量。至此 D-03 跨模块依赖边全部清零，由
    `scripts/governance/check_module_dependencies.py --check` 与边界测试防回弹。
- 处置路径（剩余）：D-03 跨模块依赖边已清零；后续只保留 gate 防回弹与新变更审查。
  结构主线转入 D-04 AI tool module_code 语义归位，不再把 D-04 混入 D-03 收口。
- 状态：**closed/gate-built（跨模块依赖边清零，环已清零）**——D-03A/D-03B/D-03C/D-03D/D-03E/D-03F/D-03G/D-03H/D-03I/D-03J/D-03K/D-03L/D-03M/D-03N/D-03O/D-03P/D-03Q/D-03R 已收口，
  基线 0 edges / 0 cycles。

### D-04 AI tool module_code 语义债 — MED · 结构

- 内容：AI tool `module_code` 直接参与运行时 `enabled_modules` 过滤；历史上大量非考试工具挤在
  `module_code="exam"`，导致模块开关表达失真：关闭 `study_analytics` / `research` 时仍可能因
  `exam` 开启而露出学情分析、学生画像、题库/知识工具。真源：
  `docs/governance/ai-tool-module-codes.yaml` + `scripts/governance/check_ai_tool_modules.py`。
- **D-04A（2026-06-22，Codex 本地全授权小窗）**：按 `docs/governance/module-semantics.yaml`
  的第一性映射，把明确归属工具归位：`analytics` / `adaptive` / `profile` domain 共 24 个工具
  从 `exam` 改为 `study_analytics`；`bank` / `knowledge` domain 共 6 个工具从 `exam` 改为
  `research`。基线 **exam 46→16、study_analytics 0→24、research 3→9**。同批给
  `check_ai_tool_modules.py` 增加 domain→module_code 语义闸门：`analytics/adaptive/profile`
  必须为 `study_analytics`，`bank/knowledge` 必须为 `research`，`homework/conduct` 继续守原归属；
  新增运行时过滤测试，确认这些工具不再随 `exam` 开关露出，而随 `study_analytics` / `research`
  开关露出。
- **D-04B（2026-06-22，Codex 本地全授权小窗）**：补齐 AI 工具模块语义表达：
  `EduToolMeta.module_code` 允许 `None` 表示基础/免控工具，并新增 `requires_modules`
  表示组合模块门槛；注册时 `filter_tools_for_role` 与调用前 `PolicyToolGuardrail`
  同步执行主模块 + 组合模块校验，`tool_gateway` 对外暴露该依赖。
  裁定：`student` 4 个工具与 `system` 4 个制品/记忆工具改为 `module_code=null`；
  `generate_report` 改为 `studio` 且 `requires_modules=["exam"]`；
  `generate_comment` 改为 `studio`；`draft_parent_notification` 保持 `conduct`
  且 `requires_modules=["studio"]`。基线 **exam 16→6、studio 0→2、null 0→8**。
  同批 `check_ai_tool_modules.py` 纳入 `requires_modules` 漂移检查，防止组合依赖静默漏审。
- 状态：**closed/gate-built（D-04A/D-04B 已收口；AI tool module_code 语义归位 + 组合门槛纳入基线）**。
...
- 内容：Portal Phase 1 = CONDITIONAL UNLOCK（`docs/archive/plans/2026-06-07-phase09-portal-unlock-decision.md`，
  sid:a4e5781a）。C1（DB 红→绿）✅、C2（四面 hash 对齐）✅、C3 线上
  fail-closed 复验 + **R-H5 生产 SchoolModule 行完整性核查** ✅；设计者/用户已在
  2026-06-22 Codex thread 签发 D-08C 并进入 D-08D。
- 处置路径：W4（C3 复验窗，read_only + 线上凭据）→ 设计者签发 → D-08D first cut
  → runtime/online closeout；Phase B/R-H2 exact-HEAD CI 已由 master run `27953800524` 关闭。
- D-08C（2026-06-22, Codex local planner）证据：
  `docs/reviews/2026-06-22-portal-c3-online-verification.md`。C1 DB doctor green；
  C2 source/build/nginx/backend `ALL ALIGNED`；治理闸门 clean；R-H5 生产
  SchoolModule 行完整性 green（3 schools x 9 rows = 27, `integrity_issues=0`）；
  线上 C3 green（禁用 `teaching`/`research`/`study_analytics` 返回 403，
  `/api/v1/portal/services` 无禁用模块泄漏，`blocked_leaks=[]`）。
- D-08D（2026-06-22, Codex local source commit `e9a9d9da`）第一刀：设计者/用户已在
  Codex thread 签发 D-08C 并进入 D-08D。首页消费全部 5 个 `/api/v1/portal/*`
  端点，Portal 待办/消息/日程成为 dashboard 派生源，service cards 经后端过滤后再由
  `moduleGateFromAuth` + permission 前端复核；未挂载前端 route 的 service 不展示，避免
  伪关闭 `studio-frontend-entry-missing`。未改 `DEFAULT_ENABLED` / middleware /
  authGuard / module-semantics。
- D-08D runtime/online closeout（2026-06-22, live HEAD `332862a0`）：local/origin/ECS
  source、frontend build、nginx、backend runtime 均对齐 `332862a0`；`truth-status.sh`
  = `ALL ALIGNED`；authenticated online probes for all five `/api/v1/portal/*`
  endpoints return HTTP `200`; `/api/v1/portal/services` returns enabled codes
  `exam`, `grading`, `homework`, `calendar`, `studio`, `conduct` with
  `blocked_leaks=[]` for disabled `teaching`/`research`/`study_analytics`.
  证据：`docs/reviews/2026-06-22-portal-d08d-runtime-closeout.md`。
- 状态：**D-08D runtime/online closed; Phase B/R-H2 exact-HEAD CI closed by later master run `27953800524` at `e52f86d`**。
  本地证据：DashboardPage targeted Vitest 41 passed；lint 0 error（1 个既有
  `ChatPanel.vue v-html` warning）；follow-up build-script hardening 后
  `npm run build` 在 Windows 本地 passed，新增 version-fingerprint + DashboardPage
  定向 Vitest 47 passed。GitHub Actions master exact-HEAD run `27953800524`
  confirmed `backend` / `frontend` / `governance` success at `e52f86d`.

### R-H1 worker freshness blind spot — HIGH · 运行态 — **closed/evidence-recorded/runtime-green**

- 处置：`f18a507` 让 worker 在启动时写 `logs/worker-runtime.json`，并把 worker git hash、
  boot time、recorded time、service PID、dirty flag 纳入 `truth-status.sh`、
  `guardian-watch`、`codex-context` 与治理测试。
- CI 证据：GitHub Actions run `27958619284` at `f18a5076f697b636fc3d758a76d974b6eccebac9`
  = `backend` / `frontend` / `governance` all success。
- live 证据：ECS 22:33 live-sync 后 source/build/nginx/backend/worker 均为 `f18a507`，
  `truth-status.sh` = `ALL ALIGNED`，`truth-doctor.sh` = `HEALTHY`；迁移陈旧 `.codex/`
  与 `data/.db_migrate.lock` 到 `/home/ops/backups/edu-cloud/rh1-runtime-artifact-cleanup-20260622T143821Z/`
  后，`guardian-watch --once --no-network --no-model-review` = `green`、0 red、0 yellow。
- 证据：`docs/reviews/2026-06-22-rh1-worker-runtime-fingerprint-closeout.md`。

### D-09 其余开放风险（指针登记，真源在 audit/调查报告）

| 项 | 级别 | 一句话 | 处置路径 |
|---|---|---|---|
| R-H1 后半 | HIGH | **closed/evidence-recorded/runtime-green**：`f18a507` 后 worker 启动写 runtime fingerprint，truthline/guardian 校验 worker git/boot/service_pid/dirty/drift；ECS live 后 guardian `green`（0 red / 0 yellow） | 证据：`docs/reviews/2026-06-22-rh1-worker-runtime-fingerprint-closeout.md` |
| R-H2 | HIGH | 全量验证层休眠（CI 实跑证据缺） | **closed/evidence-recorded**：master exact-HEAD run `27953800524` at `e52f86d` green（backend/frontend/governance） |
| R-H3 | HIGH | 前端门控面零全量回归记录（0.6→0.7A 收口全在 vitest） | **closed/evidence-recorded**：`docs/reviews/2026-06-22-rh3-frontend-gating-regression-closeout.md`；frontend full Vitest `117` files / `2525` tests passed |
| R-M1 | MED | portal 5 端点零消费者、过滤无守卫 | **closed/evidence-recorded**：source/runtime/online closed by D-08D `e9a9d9da` + `332862a0`；master exact-HEAD CI closed by `27953800524` |
| R-M5 | MED | 迁移备份 564MB/次无轮转、失败无自动回滚 | 与 Q5 周期备份一并 |
| R-M6 | MED | **closed/evidence-recorded**: after `4341271`, delivery drift verification is no longer observation-only; unknown drift / HEAD drift / source dirty delivery states fail closed | GitHub Actions `Tests` run `27994260544` green; `scripts/codex-verify full --dry-run --schema --no-network` includes delivery-chain classification |
| R-M7 | MED | **closed/gate-built**: `/uploads` StaticFiles is now in the module governance denominator as an explicit public upload asset exemption | `static_mounts` truth + `check_static_mounts` discover FastAPI StaticFiles; undeclared / stale / name-mismatch / missing expect / missing reason fail closed |
...
D-09M2 closeout note (2026-06-23): this window only adds the governance denominator; it does not change `/uploads` runtime semantics, auth policy, or file serving behavior.
First principle: every controlled surface must be enumerable. A non-API static surface may be module-gate exempt, but it must not silently sit outside module-semantics verification.
`/uploads` is therefore declared as `public_upload_asset` with `module_gate=exempt`; authorization remains owned by the API endpoints that create or reference uploaded files.
Future `StaticFiles` mounts that are undeclared, stale, name-mismatched, missing `expect`, or missing a reason fail `check_module_semantics.py --check`.
